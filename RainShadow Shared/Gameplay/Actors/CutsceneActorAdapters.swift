...
/// `RouteFollower` and the 15 Hz logic tick, which is the frozen rule from
...
extension DetectiveActorNode: CutsceneActorDriving {
    ...
    func cutsceneFace(_ facing: ActorFacing) {
        turnToFace(
            CGPoint(
                x: position.x + cos(CGFloat(facing.rawValue) * ActorFacing.sectorAngle) * 100,
                y: position.y + sin(CGFloat(facing.rawValue) * ActorFacing.sectorAngle) * 100
            )
        )
    }
    ...
    func cutsceneStandUp(completion: @escaping () -> Void) {
        walk(path: [], completion: completion)
    }
}
