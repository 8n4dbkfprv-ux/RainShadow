...
enum CityDistrictLayout {
    ...
    static var cameraScaleAt100Percent: CGFloat {
        CityDistrictDefinition.cameraScaleAt100Percent
    }

    static func cameraVisibleHeight(forSceneHeight sceneHeight: CGFloat) -> CGFloat {
        CityDistrictDefinition.cameraVisibleHeight(forSceneHeight: sceneHeight)
    }
    ...
    /// prefix vs the first `city_building_*` copy. Empty under IEAreaV04 —
    /// decorative Sable leaves retired with the continuous plate.
    ...
    /// IEAreaV04 normalizes every landmark leaf to the 80.5-wu / 161-px opening.
    enum SourceSeparateDoorLeafHeight {
        static let standard: CGFloat = 161
        static let wide: CGFloat = 161
    }
    ...
    /// With a 161-px leaf and 70.3125 adult, scale is 0.5 in SpriteKit points.
    ...
    static let aperturesByLeafTexture: [String: SourceDoorAperture] = [
        // Sable Row's enterable door is painted on the continuous plate; the
        // authoring leaf sits on CityDoorPaintedAperture, not a modular lot crop.
    ...
    /// Hit box over a painted IE aperture (threshold at the bottom, y-up).
    static func portalHitArea(
        paintedAperture aperture: CGRect,
        pad: CGFloat = 10
    ) -> CGRect {
        CGRect(
            x: aperture.minX - pad,
            y: aperture.minY - pad,
            width: aperture.width + pad * 2,
            height: aperture.height + pad * 2
        )
    }
    ...
    static func portalApproach(
        ...
        portalApproach(
            fromThreshold: leaf.groundPoint,
            clearOf: obstacles,
    ...
    /// Street-side approach from a painted doorway threshold (IE outdoor ARE).
    static func portalApproach(
        fromThreshold threshold: CGPoint,
        clearOf obstacles: [CGRect],
        minimumOffset: CGFloat = 140,
        clearance: CGFloat = 24
    ) -> CGPoint {
            ...
            for sense in senses {
                for lateral in laterals {
                    let candidate = CGPoint(
                        x: threshold.x + lateral,
                        y: threshold.y + sense * offset
                    )
        ...
        return CGPoint(x: threshold.x, y: threshold.y - minimumOffset)
    }
    ...
    ///
    /// Measured-only door leaves count. A ward that paints its closed doors into
    /// the plate draws no leaf sprite, but the leaf is still the scale authority
    /// for grading that door — dropping it would make the door ungradeable
    /// rather than correctly scaled.
    static func anyDistrictScale(forTextureName name: String) -> CGFloat? {
        for id in CityDistrictID.allCases {
            ...
            if let scale = (district.visualSprites + district.measuredDoorLeaves)
