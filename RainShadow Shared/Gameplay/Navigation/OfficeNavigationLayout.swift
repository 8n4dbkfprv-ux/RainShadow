import CoreGraphics

/// Generated by `ArtSource/Processing/office_layout_plan.py` — edit the
/// layout there and re-run, so art, navigation and previews stay in step.
///
/// Placements are authored in the shell's floor-plan basis: every prop sits on
/// a measured room axis, so furniture lines up with the painted walls.
enum OfficeNavigationLayout {
    enum Architecture {
        /// Shell-authoritative centre of the painted exterior threshold.
        static let entranceAnchor = CGPoint(x: 2_822.154, y: 1_248.909)
        /// Window insert centre on the painted left-wall recess.
        static let windowAnchor = CGPoint(x: 1_245.000, y: 1_640.000)
        /// Interior partition: one wall on the room's north-east axis (a = 0.426),
        /// with a single framed doorway as the only connection between rooms.
        static let partitionLineA: CGFloat = 0.426
        static let partitionThicknessA: CGFloat = 0.03151065801668211
        static let partitionDoorB0: CGFloat = 0.377
        static let partitionDoorB1: CGFloat = 0.533
        static let partitionReturnB1: CGFloat = 0.5630000000000001
        static let wallThicknessPx: CGFloat = 12.0
        static let axisNW = CGVector(dx: -863.2, dy: 647.4)
        static let axisNE = CGVector(dx: 1028.8, dy: 771.6)
        static let rearCorner = CGPoint(x: 1934.3, y: 1914.8)

        /// Partition plate geometry in shell art pixels (y down), so the scene can
        /// slice the painted wall into depth-sorted columns that sort like props
        /// standing on the wall's own ground line.
        static let partitionPlateX0: CGFloat = 1527
        static let partitionPlateX1: CGFloat = 2568
        static let partitionPlateFaceHeight: CGFloat = 271
        static let partitionPlateCapHeight: CGFloat = 6

        /// Ground line of the partition face at a shell-art x (y down).
        static func partitionPlateBaseY(atPlateX x: CGFloat) -> CGFloat {
            676.1 + (x - 1527) * 0.7500
        }

        /// Clear exterior opening measured from the shipping suite plate.
        static let entranceOpeningPlateSize = CGSize(width: 125.000, height: 198.0)
        /// 198 plate px × environment = 78.21 world units against a 70.31-unit adult. Matches the ~1.16× real door-to-adult ratio.
        static let entranceOpeningToDetectiveRatio: CGFloat = 1.112
        static let entranceHandleHeightToDetective: CGFloat = 0.501

        /// Exterior leaf/frame projected onto the sloped NE wall opening.
        /// Uniform height-fit (keeps sheared master aspect; no X-squash).
        static let entranceLeafDisplayScale: CGFloat = 0.1226
        static let entranceLeafDisplayScaleX: CGFloat = entranceLeafDisplayScale
        static let entranceLeafDisplayScaleY: CGFloat = entranceLeafDisplayScale
        static let entranceLeafAnchorY: CGFloat = 0.08903
        static let entranceLeafAnchor = CGPoint(x: 2_822.000, y: 1_249.000)
        static let entranceFrameDisplayScale: CGFloat = 0.1224
        static let entranceFrameDisplayScaleX: CGFloat = entranceFrameDisplayScale
        static let entranceFrameDisplayScaleY: CGFloat = entranceFrameDisplayScale
        static let entranceFrameAnchorX: CGFloat = 0.49155
        static let entranceFrameAnchorY: CGFloat = 0.11529
        /// Floor-projected presentation used after the leaf breaks free.
        static let entranceFallenLeafScaleRatio: CGFloat = 0.92
        /// The upright art grows slightly as its top swings toward the camera.
        /// This is only the transition silhouette; the landed art has an
        /// explicit world-space size below.
        static let entranceFallingTransitionScale: CGFloat =
            0.17 * OfficeInteriorScale.ActorDisplay.visualBodyRatio
        /// Purpose-built 768×512 landed-state art. The transparent source
        /// canvas stays centered so this scale yields a door body proportional
        /// to the upright leaf it fell from.
        static let entranceFallenArtworkCanvasSize = CGSize(width: 768, height: 512)
        static let entranceFallenArtworkDisplayScale: CGFloat =
            0.17 * OfficeInteriorScale.ActorDisplay.visualBodyRatio
        static let entranceFallenArtworkDisplaySize = CGSize(
            width: entranceFallenArtworkCanvasSize.width
                * entranceFallenArtworkDisplayScale,
            height: entranceFallenArtworkCanvasSize.height
                * entranceFallenArtworkDisplayScale
        )
        /// Internal open leaf registered to the shipping partition hinge.
        static let internalHingePlateX: CGFloat = 1955.0
        static let internalHingePlateHeight: CGFloat = 290.0
        /// Height-fit to `internalHingePlateHeight` × environment / hinge texture.
        static let internalLeafDisplayScale: CGFloat = 0.3311
        static let internalLeafAnchor = CGPoint(x: 1880.824, y: 1299.119)
    }

    /// Walkable chair-side stand point (camera-near of the kneehole).
    /// Kept outside the desk obstacle so leave-seat never starts on the
    /// visitor/rear side of the writing surface.
    private static let authoredActorStart = CGPoint(
        x: AuthoredPlacement.deskChair.x,
        y: AuthoredPlacement.deskChair.y - 30
    )

    // MARK: - Obstacles (authored AABBs around each floor footprint)

    /// One cell of the camera-near boundary, kept named for the layout tests.
    static let authoredForegroundWallObstacle = CGRect(x: 2502, y: 828, width: 104, height: 78)
    static let authoredDoorObstacle = CGRect(x: 2716, y: 1180, width: 212, height: 139)

    /// Room boundary: the rear and side walls plus the camera-near cutaway
    /// wall, one solid per navigation cell outside the painted floor.
    static let authoredBoundarySegments: [CGRect] = [
        CGRect(x: 979, y: 1156, width: 41, height: 30),
        CGRect(x: 979, y: 1186, width: 41, height: 30),
        CGRect(x: 979, y: 1217, width: 41, height: 30),
        CGRect(x: 979, y: 1247, width: 41, height: 30),
        CGRect(x: 979, y: 1277, width: 41, height: 30),
        CGRect(x: 979, y: 1308, width: 41, height: 30),
        CGRect(x: 979, y: 1338, width: 41, height: 30),
        CGRect(x: 1019, y: 1125, width: 41, height: 30),
        CGRect(x: 1019, y: 1156, width: 41, height: 30),
        CGRect(x: 1019, y: 1186, width: 41, height: 30),
        CGRect(x: 1019, y: 1217, width: 41, height: 30),
        CGRect(x: 1019, y: 1247, width: 41, height: 30),
        CGRect(x: 1019, y: 1277, width: 41, height: 30),
        CGRect(x: 1019, y: 1308, width: 41, height: 30),
        CGRect(x: 1019, y: 1338, width: 41, height: 30),
        CGRect(x: 1019, y: 1369, width: 41, height: 30),
        CGRect(x: 1060, y: 1095, width: 41, height: 30),
        CGRect(x: 1060, y: 1125, width: 41, height: 30),
        CGRect(x: 1060, y: 1156, width: 41, height: 30),
        CGRect(x: 1060, y: 1186, width: 41, height: 30),
        CGRect(x: 1060, y: 1217, width: 41, height: 30),
        CGRect(x: 1060, y: 1247, width: 41, height: 30),
        CGRect(x: 1060, y: 1277, width: 41, height: 30),
        CGRect(x: 1060, y: 1308, width: 41, height: 30),
        CGRect(x: 1060, y: 1338, width: 41, height: 30),
        CGRect(x: 1060, y: 1369, width: 41, height: 30),
        CGRect(x: 1060, y: 1399, width: 41, height: 30),
        CGRect(x: 1100, y: 1065, width: 41, height: 30),
        CGRect(x: 1100, y: 1095, width: 41, height: 30),
        CGRect(x: 1100, y: 1125, width: 41, height: 30),
        CGRect(x: 1100, y: 1156, width: 41, height: 30),
        CGRect(x: 1100, y: 1186, width: 41, height: 30),
        CGRect(x: 1100, y: 1217, width: 41, height: 30),
        CGRect(x: 1100, y: 1277, width: 41, height: 30),
        CGRect(x: 1100, y: 1308, width: 41, height: 30),
        CGRect(x: 1100, y: 1338, width: 41, height: 30),
        CGRect(x: 1100, y: 1369, width: 41, height: 30),
        CGRect(x: 1100, y: 1399, width: 41, height: 30),
        CGRect(x: 1100, y: 1429, width: 41, height: 30),
        CGRect(x: 1141, y: 1034, width: 41, height: 30),
        CGRect(x: 1141, y: 1065, width: 41, height: 30),
        CGRect(x: 1141, y: 1095, width: 41, height: 30),
        CGRect(x: 1141, y: 1125, width: 41, height: 30),
        CGRect(x: 1141, y: 1156, width: 41, height: 30),
        CGRect(x: 1141, y: 1186, width: 41, height: 30),
        CGRect(x: 1141, y: 1308, width: 41, height: 30),
        CGRect(x: 1141, y: 1338, width: 41, height: 30),
        CGRect(x: 1141, y: 1369, width: 41, height: 30),
        CGRect(x: 1141, y: 1399, width: 41, height: 30),
        CGRect(x: 1141, y: 1429, width: 41, height: 30),
        CGRect(x: 1141, y: 1460, width: 41, height: 30),
        CGRect(x: 1181, y: 1004, width: 41, height: 30),
        CGRect(x: 1181, y: 1034, width: 41, height: 30),
        CGRect(x: 1181, y: 1065, width: 41, height: 30),
        CGRect(x: 1181, y: 1095, width: 41, height: 30),
        CGRect(x: 1181, y: 1125, width: 41, height: 30),
        CGRect(x: 1181, y: 1156, width: 41, height: 30),
        CGRect(x: 1181, y: 1338, width: 41, height: 30),
        CGRect(x: 1181, y: 1369, width: 41, height: 30),
        CGRect(x: 1181, y: 1399, width: 41, height: 30),
        CGRect(x: 1181, y: 1429, width: 41, height: 30),
        CGRect(x: 1181, y: 1460, width: 41, height: 30),
        CGRect(x: 1181, y: 1490, width: 41, height: 30),
        CGRect(x: 1222, y: 974, width: 41, height: 30),
        CGRect(x: 1222, y: 1004, width: 41, height: 30),
        CGRect(x: 1222, y: 1034, width: 41, height: 30),
        CGRect(x: 1222, y: 1065, width: 41, height: 30),
        CGRect(x: 1222, y: 1095, width: 41, height: 30),
        CGRect(x: 1222, y: 1125, width: 41, height: 30),
        CGRect(x: 1222, y: 1369, width: 41, height: 30),
        CGRect(x: 1222, y: 1399, width: 41, height: 30),
        CGRect(x: 1222, y: 1429, width: 41, height: 30),
        CGRect(x: 1222, y: 1460, width: 41, height: 30),
        CGRect(x: 1222, y: 1490, width: 41, height: 30),
        CGRect(x: 1222, y: 1520, width: 41, height: 30),
        CGRect(x: 1262, y: 943, width: 41, height: 30),
        CGRect(x: 1262, y: 974, width: 41, height: 30),
        CGRect(x: 1262, y: 1004, width: 41, height: 30),
        CGRect(x: 1262, y: 1034, width: 41, height: 30),
        CGRect(x: 1262, y: 1065, width: 41, height: 30),
        CGRect(x: 1262, y: 1095, width: 41, height: 30),
        CGRect(x: 1262, y: 1399, width: 41, height: 30),
        CGRect(x: 1262, y: 1429, width: 41, height: 30),
        CGRect(x: 1262, y: 1460, width: 41, height: 30),
        CGRect(x: 1262, y: 1490, width: 41, height: 30),
        CGRect(x: 1262, y: 1520, width: 41, height: 30),
        CGRect(x: 1262, y: 1551, width: 41, height: 30),
        CGRect(x: 1303, y: 913, width: 41, height: 30),
        CGRect(x: 1303, y: 943, width: 41, height: 30),
        CGRect(x: 1303, y: 974, width: 41, height: 30),
        CGRect(x: 1303, y: 1004, width: 41, height: 30),
        CGRect(x: 1303, y: 1034, width: 41, height: 30),
        CGRect(x: 1303, y: 1065, width: 41, height: 30),
        CGRect(x: 1303, y: 1429, width: 41, height: 30),
        CGRect(x: 1303, y: 1460, width: 41, height: 30),
        CGRect(x: 1303, y: 1490, width: 41, height: 30),
        CGRect(x: 1303, y: 1520, width: 41, height: 30),
        CGRect(x: 1303, y: 1551, width: 41, height: 30),
        CGRect(x: 1303, y: 1581, width: 41, height: 30),
        CGRect(x: 1343, y: 882, width: 41, height: 30),
        CGRect(x: 1343, y: 913, width: 41, height: 30),
        CGRect(x: 1343, y: 943, width: 41, height: 30),
        CGRect(x: 1343, y: 974, width: 41, height: 30),
        CGRect(x: 1343, y: 1004, width: 41, height: 30),
        CGRect(x: 1343, y: 1034, width: 41, height: 30),
        CGRect(x: 1343, y: 1460, width: 41, height: 30),
        CGRect(x: 1343, y: 1490, width: 41, height: 30),
        CGRect(x: 1343, y: 1520, width: 41, height: 30),
        CGRect(x: 1343, y: 1551, width: 41, height: 30),
        CGRect(x: 1343, y: 1581, width: 41, height: 30),
        CGRect(x: 1343, y: 1612, width: 41, height: 30),
        CGRect(x: 1384, y: 852, width: 41, height: 30),
        CGRect(x: 1384, y: 882, width: 41, height: 30),
        CGRect(x: 1384, y: 913, width: 41, height: 30),
        CGRect(x: 1384, y: 943, width: 41, height: 30),
        CGRect(x: 1384, y: 974, width: 41, height: 30),
        CGRect(x: 1384, y: 1004, width: 41, height: 30),
        CGRect(x: 1384, y: 1490, width: 41, height: 30),
        CGRect(x: 1384, y: 1520, width: 41, height: 30),
        CGRect(x: 1384, y: 1551, width: 41, height: 30),
        CGRect(x: 1384, y: 1581, width: 41, height: 30),
        CGRect(x: 1384, y: 1612, width: 41, height: 30),
        CGRect(x: 1384, y: 1642, width: 41, height: 30),
        CGRect(x: 1424, y: 822, width: 41, height: 30),
        CGRect(x: 1424, y: 852, width: 41, height: 30),
        CGRect(x: 1424, y: 882, width: 41, height: 30),
        CGRect(x: 1424, y: 913, width: 41, height: 30),
        CGRect(x: 1424, y: 943, width: 41, height: 30),
        CGRect(x: 1424, y: 974, width: 41, height: 30),
        CGRect(x: 1424, y: 1520, width: 41, height: 30),
        CGRect(x: 1424, y: 1551, width: 41, height: 30),
        CGRect(x: 1424, y: 1581, width: 41, height: 30),
        CGRect(x: 1424, y: 1612, width: 41, height: 30),
        CGRect(x: 1424, y: 1642, width: 41, height: 30),
        CGRect(x: 1424, y: 1672, width: 41, height: 30),
        CGRect(x: 1465, y: 791, width: 41, height: 30),
        CGRect(x: 1465, y: 822, width: 41, height: 30),
        CGRect(x: 1465, y: 852, width: 41, height: 30),
        CGRect(x: 1465, y: 882, width: 41, height: 30),
        CGRect(x: 1465, y: 913, width: 41, height: 30),
        CGRect(x: 1465, y: 943, width: 41, height: 30),
        CGRect(x: 1465, y: 1551, width: 41, height: 30),
        CGRect(x: 1465, y: 1581, width: 41, height: 30),
        CGRect(x: 1465, y: 1612, width: 41, height: 30),
        CGRect(x: 1465, y: 1642, width: 41, height: 30),
        CGRect(x: 1465, y: 1672, width: 41, height: 30),
        CGRect(x: 1465, y: 1703, width: 41, height: 30),
        CGRect(x: 1505, y: 761, width: 41, height: 30),
        CGRect(x: 1505, y: 791, width: 41, height: 30),
        CGRect(x: 1505, y: 822, width: 41, height: 30),
        CGRect(x: 1505, y: 852, width: 41, height: 30),
        CGRect(x: 1505, y: 882, width: 41, height: 30),
        CGRect(x: 1505, y: 913, width: 41, height: 30),
        CGRect(x: 1505, y: 1581, width: 41, height: 30),
        CGRect(x: 1505, y: 1612, width: 41, height: 30),
        CGRect(x: 1505, y: 1642, width: 41, height: 30),
        CGRect(x: 1505, y: 1672, width: 41, height: 30),
        CGRect(x: 1505, y: 1703, width: 41, height: 30),
        CGRect(x: 1505, y: 1733, width: 41, height: 30),
        CGRect(x: 1546, y: 731, width: 41, height: 30),
        CGRect(x: 1546, y: 761, width: 41, height: 30),
        CGRect(x: 1546, y: 791, width: 41, height: 30),
        CGRect(x: 1546, y: 822, width: 41, height: 30),
        CGRect(x: 1546, y: 852, width: 41, height: 30),
        CGRect(x: 1546, y: 882, width: 41, height: 30),
        CGRect(x: 1546, y: 1612, width: 41, height: 30),
        CGRect(x: 1546, y: 1642, width: 41, height: 30),
        CGRect(x: 1546, y: 1672, width: 41, height: 30),
        CGRect(x: 1546, y: 1703, width: 41, height: 30),
        CGRect(x: 1546, y: 1733, width: 41, height: 30),
        CGRect(x: 1546, y: 1763, width: 41, height: 30),
        CGRect(x: 1586, y: 700, width: 41, height: 30),
        CGRect(x: 1586, y: 731, width: 41, height: 30),
        CGRect(x: 1586, y: 761, width: 41, height: 30),
        CGRect(x: 1586, y: 791, width: 41, height: 30),
        CGRect(x: 1586, y: 822, width: 41, height: 30),
        CGRect(x: 1586, y: 852, width: 41, height: 30),
        CGRect(x: 1586, y: 1642, width: 41, height: 30),
        CGRect(x: 1586, y: 1672, width: 41, height: 30),
        CGRect(x: 1586, y: 1703, width: 41, height: 30),
        CGRect(x: 1586, y: 1733, width: 41, height: 30),
        CGRect(x: 1586, y: 1763, width: 41, height: 30),
        CGRect(x: 1586, y: 1794, width: 41, height: 30),
        CGRect(x: 1627, y: 670, width: 41, height: 30),
        CGRect(x: 1627, y: 700, width: 41, height: 30),
        CGRect(x: 1627, y: 731, width: 41, height: 30),
        CGRect(x: 1627, y: 761, width: 41, height: 30),
        CGRect(x: 1627, y: 791, width: 41, height: 30),
        CGRect(x: 1627, y: 822, width: 41, height: 30),
        CGRect(x: 1627, y: 1672, width: 41, height: 30),
        CGRect(x: 1627, y: 1703, width: 41, height: 30),
        CGRect(x: 1627, y: 1733, width: 41, height: 30),
        CGRect(x: 1627, y: 1763, width: 41, height: 30),
        CGRect(x: 1627, y: 1794, width: 41, height: 30),
        CGRect(x: 1627, y: 1824, width: 41, height: 30),
        CGRect(x: 1667, y: 639, width: 41, height: 30),
        CGRect(x: 1667, y: 670, width: 41, height: 30),
        CGRect(x: 1667, y: 700, width: 41, height: 30),
        CGRect(x: 1667, y: 731, width: 41, height: 30),
        CGRect(x: 1667, y: 761, width: 41, height: 30),
        CGRect(x: 1667, y: 791, width: 41, height: 30),
        CGRect(x: 1667, y: 1703, width: 41, height: 30),
        CGRect(x: 1667, y: 1733, width: 41, height: 30),
        CGRect(x: 1667, y: 1763, width: 41, height: 30),
        CGRect(x: 1667, y: 1794, width: 41, height: 30),
        CGRect(x: 1667, y: 1824, width: 41, height: 30),
        CGRect(x: 1667, y: 1855, width: 41, height: 30),
        CGRect(x: 1708, y: 609, width: 41, height: 30),
        CGRect(x: 1708, y: 639, width: 41, height: 30),
        CGRect(x: 1708, y: 670, width: 41, height: 30),
        CGRect(x: 1708, y: 700, width: 41, height: 30),
        CGRect(x: 1708, y: 731, width: 41, height: 30),
        CGRect(x: 1708, y: 761, width: 41, height: 30),
        CGRect(x: 1708, y: 1733, width: 41, height: 30),
        CGRect(x: 1708, y: 1763, width: 41, height: 30),
        CGRect(x: 1708, y: 1794, width: 41, height: 30),
        CGRect(x: 1708, y: 1824, width: 41, height: 30),
        CGRect(x: 1708, y: 1855, width: 41, height: 30),
        CGRect(x: 1708, y: 1885, width: 41, height: 30),
        CGRect(x: 1748, y: 579, width: 41, height: 30),
        CGRect(x: 1748, y: 609, width: 41, height: 30),
        CGRect(x: 1748, y: 639, width: 41, height: 30),
        CGRect(x: 1748, y: 670, width: 41, height: 30),
        CGRect(x: 1748, y: 700, width: 41, height: 30),
        CGRect(x: 1748, y: 731, width: 41, height: 30),
        CGRect(x: 1748, y: 1763, width: 41, height: 30),
        CGRect(x: 1748, y: 1794, width: 41, height: 30),
        CGRect(x: 1748, y: 1824, width: 41, height: 30),
        CGRect(x: 1748, y: 1855, width: 41, height: 30),
        CGRect(x: 1748, y: 1885, width: 41, height: 30),
        CGRect(x: 1748, y: 1915, width: 41, height: 30),
        CGRect(x: 1789, y: 548, width: 41, height: 30),
        CGRect(x: 1789, y: 579, width: 41, height: 30),
        CGRect(x: 1789, y: 609, width: 41, height: 30),
        CGRect(x: 1789, y: 639, width: 41, height: 30),
        CGRect(x: 1789, y: 670, width: 41, height: 30),
        CGRect(x: 1789, y: 700, width: 41, height: 30),
        CGRect(x: 1789, y: 1794, width: 41, height: 30),
        CGRect(x: 1789, y: 1824, width: 41, height: 30),
        CGRect(x: 1789, y: 1855, width: 41, height: 30),
        CGRect(x: 1789, y: 1885, width: 41, height: 30),
        CGRect(x: 1789, y: 1915, width: 41, height: 30),
        CGRect(x: 1789, y: 1946, width: 41, height: 30),
        CGRect(x: 1829, y: 518, width: 41, height: 30),
        CGRect(x: 1829, y: 548, width: 41, height: 30),
        CGRect(x: 1829, y: 579, width: 41, height: 30),
        CGRect(x: 1829, y: 609, width: 41, height: 30),
        CGRect(x: 1829, y: 639, width: 41, height: 30),
        CGRect(x: 1829, y: 670, width: 41, height: 30),
        CGRect(x: 1829, y: 1824, width: 41, height: 30),
        CGRect(x: 1829, y: 1855, width: 41, height: 30),
        CGRect(x: 1829, y: 1885, width: 41, height: 30),
        CGRect(x: 1829, y: 1915, width: 41, height: 30),
        CGRect(x: 1829, y: 1946, width: 41, height: 30),
        CGRect(x: 1870, y: 488, width: 41, height: 30),
        CGRect(x: 1870, y: 518, width: 41, height: 30),
        CGRect(x: 1870, y: 548, width: 41, height: 30),
        CGRect(x: 1870, y: 579, width: 41, height: 30),
        CGRect(x: 1870, y: 609, width: 41, height: 30),
        CGRect(x: 1870, y: 639, width: 41, height: 30),
        CGRect(x: 1870, y: 1855, width: 41, height: 30),
        CGRect(x: 1870, y: 1885, width: 41, height: 30),
        CGRect(x: 1870, y: 1915, width: 41, height: 30),
        CGRect(x: 1870, y: 1946, width: 41, height: 30),
        CGRect(x: 1910, y: 457, width: 41, height: 30),
        CGRect(x: 1910, y: 488, width: 41, height: 30),
        CGRect(x: 1910, y: 518, width: 41, height: 30),
        CGRect(x: 1910, y: 548, width: 41, height: 30),
        CGRect(x: 1910, y: 579, width: 41, height: 30),
        CGRect(x: 1910, y: 609, width: 41, height: 30),
        CGRect(x: 1910, y: 1885, width: 41, height: 30),
        CGRect(x: 1910, y: 1915, width: 41, height: 30),
        CGRect(x: 1910, y: 1946, width: 41, height: 30),
        CGRect(x: 1951, y: 457, width: 41, height: 30),
        CGRect(x: 1951, y: 488, width: 41, height: 30),
        CGRect(x: 1951, y: 518, width: 41, height: 30),
        CGRect(x: 1951, y: 548, width: 41, height: 30),
        CGRect(x: 1951, y: 579, width: 41, height: 30),
        CGRect(x: 1951, y: 1855, width: 41, height: 30),
        CGRect(x: 1951, y: 1885, width: 41, height: 30),
        CGRect(x: 1951, y: 1915, width: 41, height: 30),
        CGRect(x: 1951, y: 1946, width: 41, height: 30),
        CGRect(x: 1991, y: 457, width: 41, height: 30),
        CGRect(x: 1991, y: 488, width: 41, height: 30),
        CGRect(x: 1991, y: 518, width: 41, height: 30),
        CGRect(x: 1991, y: 548, width: 41, height: 30),
        CGRect(x: 1991, y: 1824, width: 41, height: 30),
        CGRect(x: 1991, y: 1855, width: 41, height: 30),
        CGRect(x: 1991, y: 1885, width: 41, height: 30),
        CGRect(x: 1991, y: 1915, width: 41, height: 30),
        CGRect(x: 1991, y: 1946, width: 41, height: 30),
        CGRect(x: 2032, y: 457, width: 41, height: 30),
        CGRect(x: 2032, y: 488, width: 41, height: 30),
        CGRect(x: 2032, y: 518, width: 41, height: 30),
        CGRect(x: 2032, y: 1794, width: 41, height: 30),
        CGRect(x: 2032, y: 1824, width: 41, height: 30),
        CGRect(x: 2032, y: 1855, width: 41, height: 30),
        CGRect(x: 2032, y: 1885, width: 41, height: 30),
        CGRect(x: 2032, y: 1915, width: 41, height: 30),
        CGRect(x: 2032, y: 1946, width: 41, height: 30),
        CGRect(x: 2072, y: 457, width: 41, height: 30),
        CGRect(x: 2072, y: 488, width: 41, height: 30),
        CGRect(x: 2072, y: 518, width: 41, height: 30),
        CGRect(x: 2072, y: 1763, width: 41, height: 30),
        CGRect(x: 2072, y: 1794, width: 41, height: 30),
        CGRect(x: 2072, y: 1824, width: 41, height: 30),
        CGRect(x: 2072, y: 1855, width: 41, height: 30),
        CGRect(x: 2072, y: 1885, width: 41, height: 30),
        CGRect(x: 2072, y: 1915, width: 41, height: 30),
        CGRect(x: 2113, y: 457, width: 41, height: 30),
        CGRect(x: 2113, y: 488, width: 41, height: 30),
        CGRect(x: 2113, y: 518, width: 41, height: 30),
        CGRect(x: 2113, y: 548, width: 41, height: 30),
        CGRect(x: 2113, y: 1733, width: 41, height: 30),
        CGRect(x: 2113, y: 1763, width: 41, height: 30),
        CGRect(x: 2113, y: 1794, width: 41, height: 30),
        CGRect(x: 2113, y: 1824, width: 41, height: 30),
        CGRect(x: 2113, y: 1855, width: 41, height: 30),
        CGRect(x: 2113, y: 1885, width: 41, height: 30),
        CGRect(x: 2153, y: 457, width: 41, height: 30),
        CGRect(x: 2153, y: 488, width: 41, height: 30),
        CGRect(x: 2153, y: 518, width: 41, height: 30),
        CGRect(x: 2153, y: 548, width: 41, height: 30),
        CGRect(x: 2153, y: 579, width: 41, height: 30),
        CGRect(x: 2153, y: 1703, width: 41, height: 30),
        CGRect(x: 2153, y: 1733, width: 41, height: 30),
        CGRect(x: 2153, y: 1763, width: 41, height: 30),
        CGRect(x: 2153, y: 1794, width: 41, height: 30),
        CGRect(x: 2153, y: 1824, width: 41, height: 30),
        CGRect(x: 2153, y: 1855, width: 41, height: 30),
        CGRect(x: 2194, y: 457, width: 41, height: 30),
        CGRect(x: 2194, y: 488, width: 41, height: 30),
        CGRect(x: 2194, y: 518, width: 41, height: 30),
        CGRect(x: 2194, y: 548, width: 41, height: 30),
        CGRect(x: 2194, y: 579, width: 41, height: 30),
        CGRect(x: 2194, y: 609, width: 41, height: 30),
        CGRect(x: 2194, y: 1672, width: 41, height: 30),
        CGRect(x: 2194, y: 1703, width: 41, height: 30),
        CGRect(x: 2194, y: 1733, width: 41, height: 30),
        CGRect(x: 2194, y: 1763, width: 41, height: 30),
        CGRect(x: 2194, y: 1794, width: 41, height: 30),
        CGRect(x: 2194, y: 1824, width: 41, height: 30),
        CGRect(x: 2234, y: 488, width: 41, height: 30),
        CGRect(x: 2234, y: 518, width: 41, height: 30),
        CGRect(x: 2234, y: 548, width: 41, height: 30),
        CGRect(x: 2234, y: 579, width: 41, height: 30),
        CGRect(x: 2234, y: 609, width: 41, height: 30),
        CGRect(x: 2234, y: 639, width: 41, height: 30),
        CGRect(x: 2234, y: 1642, width: 41, height: 30),
        CGRect(x: 2234, y: 1672, width: 41, height: 30),
        CGRect(x: 2234, y: 1703, width: 41, height: 30),
        CGRect(x: 2234, y: 1733, width: 41, height: 30),
        CGRect(x: 2234, y: 1763, width: 41, height: 30),
        CGRect(x: 2234, y: 1794, width: 41, height: 30),
        CGRect(x: 2275, y: 518, width: 41, height: 30),
        CGRect(x: 2275, y: 548, width: 41, height: 30),
        CGRect(x: 2275, y: 579, width: 41, height: 30),
        CGRect(x: 2275, y: 609, width: 41, height: 30),
        CGRect(x: 2275, y: 639, width: 41, height: 30),
        CGRect(x: 2275, y: 670, width: 41, height: 30),
        CGRect(x: 2275, y: 1612, width: 41, height: 30),
        CGRect(x: 2275, y: 1642, width: 41, height: 30),
        CGRect(x: 2275, y: 1672, width: 41, height: 30),
        CGRect(x: 2275, y: 1703, width: 41, height: 30),
        CGRect(x: 2275, y: 1733, width: 41, height: 30),
        CGRect(x: 2275, y: 1763, width: 41, height: 30),
        CGRect(x: 2315, y: 548, width: 41, height: 30),
        CGRect(x: 2315, y: 579, width: 41, height: 30),
        CGRect(x: 2315, y: 609, width: 41, height: 30),
        CGRect(x: 2315, y: 639, width: 41, height: 30),
        CGRect(x: 2315, y: 670, width: 41, height: 30),
        CGRect(x: 2315, y: 700, width: 41, height: 30),
        CGRect(x: 2315, y: 1581, width: 41, height: 30),
        CGRect(x: 2315, y: 1612, width: 41, height: 30),
        CGRect(x: 2315, y: 1642, width: 41, height: 30),
        CGRect(x: 2315, y: 1672, width: 41, height: 30),
        CGRect(x: 2315, y: 1703, width: 41, height: 30),
        CGRect(x: 2315, y: 1733, width: 41, height: 30),
        CGRect(x: 2356, y: 579, width: 41, height: 30),
        CGRect(x: 2356, y: 609, width: 41, height: 30),
        CGRect(x: 2356, y: 639, width: 41, height: 30),
        CGRect(x: 2356, y: 670, width: 41, height: 30),
        CGRect(x: 2356, y: 700, width: 41, height: 30),
        CGRect(x: 2356, y: 731, width: 41, height: 30),
        CGRect(x: 2356, y: 1551, width: 41, height: 30),
        CGRect(x: 2356, y: 1581, width: 41, height: 30),
        CGRect(x: 2356, y: 1612, width: 41, height: 30),
        CGRect(x: 2356, y: 1642, width: 41, height: 30),
        CGRect(x: 2356, y: 1672, width: 41, height: 30),
        CGRect(x: 2356, y: 1703, width: 41, height: 30),
        CGRect(x: 2396, y: 609, width: 41, height: 30),
        CGRect(x: 2396, y: 639, width: 41, height: 30),
        CGRect(x: 2396, y: 670, width: 41, height: 30),
        CGRect(x: 2396, y: 700, width: 41, height: 30),
        CGRect(x: 2396, y: 731, width: 41, height: 30),
        CGRect(x: 2396, y: 761, width: 41, height: 30),
        CGRect(x: 2396, y: 1520, width: 41, height: 30),
        CGRect(x: 2396, y: 1551, width: 41, height: 30),
        CGRect(x: 2396, y: 1581, width: 41, height: 30),
        CGRect(x: 2396, y: 1612, width: 41, height: 30),
        CGRect(x: 2396, y: 1642, width: 41, height: 30),
        CGRect(x: 2396, y: 1672, width: 41, height: 30),
        CGRect(x: 2437, y: 639, width: 41, height: 30),
        CGRect(x: 2437, y: 670, width: 41, height: 30),
        CGRect(x: 2437, y: 700, width: 41, height: 30),
        CGRect(x: 2437, y: 731, width: 41, height: 30),
        CGRect(x: 2437, y: 761, width: 41, height: 30),
        CGRect(x: 2437, y: 791, width: 41, height: 30),
        CGRect(x: 2437, y: 1490, width: 41, height: 30),
        CGRect(x: 2437, y: 1520, width: 41, height: 30),
        CGRect(x: 2437, y: 1551, width: 41, height: 30),
        CGRect(x: 2437, y: 1581, width: 41, height: 30),
        CGRect(x: 2437, y: 1612, width: 41, height: 30),
        CGRect(x: 2437, y: 1642, width: 41, height: 30),
        CGRect(x: 2477, y: 670, width: 41, height: 30),
        CGRect(x: 2477, y: 700, width: 41, height: 30),
        CGRect(x: 2477, y: 731, width: 41, height: 30),
        CGRect(x: 2477, y: 761, width: 41, height: 30),
        CGRect(x: 2477, y: 791, width: 41, height: 30),
        CGRect(x: 2477, y: 822, width: 41, height: 30),
        CGRect(x: 2477, y: 1460, width: 41, height: 30),
        CGRect(x: 2477, y: 1490, width: 41, height: 30),
        CGRect(x: 2477, y: 1520, width: 41, height: 30),
        CGRect(x: 2477, y: 1551, width: 41, height: 30),
        CGRect(x: 2477, y: 1581, width: 41, height: 30),
        CGRect(x: 2477, y: 1612, width: 41, height: 30),
        CGRect(x: 2518, y: 700, width: 41, height: 30),
        CGRect(x: 2518, y: 731, width: 41, height: 30),
        CGRect(x: 2518, y: 761, width: 41, height: 30),
        CGRect(x: 2518, y: 791, width: 41, height: 30),
        CGRect(x: 2518, y: 822, width: 41, height: 30),
        CGRect(x: 2518, y: 852, width: 41, height: 30),
        CGRect(x: 2518, y: 1429, width: 41, height: 30),
        CGRect(x: 2518, y: 1460, width: 41, height: 30),
        CGRect(x: 2518, y: 1490, width: 41, height: 30),
        CGRect(x: 2518, y: 1520, width: 41, height: 30),
        CGRect(x: 2518, y: 1551, width: 41, height: 30),
        CGRect(x: 2518, y: 1581, width: 41, height: 30),
        CGRect(x: 2558, y: 731, width: 41, height: 30),
        CGRect(x: 2558, y: 761, width: 41, height: 30),
        CGRect(x: 2558, y: 791, width: 41, height: 30),
        CGRect(x: 2558, y: 822, width: 41, height: 30),
        CGRect(x: 2558, y: 852, width: 41, height: 30),
        CGRect(x: 2558, y: 882, width: 41, height: 30),
        CGRect(x: 2558, y: 1399, width: 41, height: 30),
        CGRect(x: 2558, y: 1429, width: 41, height: 30),
        CGRect(x: 2558, y: 1460, width: 41, height: 30),
        CGRect(x: 2558, y: 1490, width: 41, height: 30),
        CGRect(x: 2558, y: 1520, width: 41, height: 30),
        CGRect(x: 2558, y: 1551, width: 41, height: 30),
        CGRect(x: 2599, y: 761, width: 41, height: 30),
        CGRect(x: 2599, y: 791, width: 41, height: 30),
        CGRect(x: 2599, y: 822, width: 41, height: 30),
        CGRect(x: 2599, y: 852, width: 41, height: 30),
        CGRect(x: 2599, y: 882, width: 41, height: 30),
        CGRect(x: 2599, y: 913, width: 41, height: 30),
        CGRect(x: 2599, y: 1369, width: 41, height: 30),
        CGRect(x: 2599, y: 1399, width: 41, height: 30),
        CGRect(x: 2599, y: 1429, width: 41, height: 30),
        CGRect(x: 2599, y: 1460, width: 41, height: 30),
        CGRect(x: 2599, y: 1490, width: 41, height: 30),
        CGRect(x: 2599, y: 1520, width: 41, height: 30),
        CGRect(x: 2639, y: 791, width: 41, height: 30),
        CGRect(x: 2639, y: 822, width: 41, height: 30),
        CGRect(x: 2639, y: 852, width: 41, height: 30),
        CGRect(x: 2639, y: 882, width: 41, height: 30),
        CGRect(x: 2639, y: 913, width: 41, height: 30),
        CGRect(x: 2639, y: 943, width: 41, height: 30),
        CGRect(x: 2639, y: 1338, width: 41, height: 30),
        CGRect(x: 2639, y: 1369, width: 41, height: 30),
        CGRect(x: 2639, y: 1399, width: 41, height: 30),
        CGRect(x: 2639, y: 1429, width: 41, height: 30),
        CGRect(x: 2639, y: 1460, width: 41, height: 30),
        CGRect(x: 2639, y: 1490, width: 41, height: 30),
        CGRect(x: 2680, y: 822, width: 41, height: 30),
        CGRect(x: 2680, y: 852, width: 41, height: 30),
        CGRect(x: 2680, y: 882, width: 41, height: 30),
        CGRect(x: 2680, y: 913, width: 41, height: 30),
        CGRect(x: 2680, y: 943, width: 41, height: 30),
        CGRect(x: 2680, y: 974, width: 41, height: 30),
        CGRect(x: 2680, y: 1308, width: 41, height: 30),
        CGRect(x: 2680, y: 1338, width: 41, height: 30),
        CGRect(x: 2680, y: 1369, width: 41, height: 30),
        CGRect(x: 2680, y: 1399, width: 41, height: 30),
        CGRect(x: 2680, y: 1429, width: 41, height: 30),
        CGRect(x: 2680, y: 1460, width: 41, height: 30),
        CGRect(x: 2720, y: 852, width: 41, height: 30),
        CGRect(x: 2720, y: 882, width: 41, height: 30),
        CGRect(x: 2720, y: 913, width: 41, height: 30),
        CGRect(x: 2720, y: 943, width: 41, height: 30),
        CGRect(x: 2720, y: 974, width: 41, height: 30),
        CGRect(x: 2720, y: 1004, width: 41, height: 30),
        CGRect(x: 2720, y: 1277, width: 41, height: 30),
        CGRect(x: 2720, y: 1308, width: 41, height: 30),
        CGRect(x: 2720, y: 1338, width: 41, height: 30),
        CGRect(x: 2720, y: 1369, width: 41, height: 30),
        CGRect(x: 2720, y: 1399, width: 41, height: 30),
        CGRect(x: 2720, y: 1429, width: 41, height: 30),
        CGRect(x: 2761, y: 882, width: 41, height: 30),
        CGRect(x: 2761, y: 913, width: 41, height: 30),
        CGRect(x: 2761, y: 943, width: 41, height: 30),
        CGRect(x: 2761, y: 974, width: 41, height: 30),
        CGRect(x: 2761, y: 1004, width: 41, height: 30),
        CGRect(x: 2761, y: 1034, width: 41, height: 30),
        CGRect(x: 2761, y: 1247, width: 41, height: 30),
        CGRect(x: 2761, y: 1277, width: 41, height: 30),
        CGRect(x: 2761, y: 1308, width: 41, height: 30),
        CGRect(x: 2761, y: 1338, width: 41, height: 30),
        CGRect(x: 2761, y: 1369, width: 41, height: 30),
        CGRect(x: 2761, y: 1399, width: 41, height: 30),
        CGRect(x: 2801, y: 913, width: 41, height: 30),
        CGRect(x: 2801, y: 943, width: 41, height: 30),
        CGRect(x: 2801, y: 974, width: 41, height: 30),
        CGRect(x: 2801, y: 1004, width: 41, height: 30),
        CGRect(x: 2801, y: 1034, width: 41, height: 30),
        CGRect(x: 2801, y: 1065, width: 41, height: 30),
        CGRect(x: 2801, y: 1217, width: 41, height: 30),
        CGRect(x: 2801, y: 1247, width: 41, height: 30),
        CGRect(x: 2801, y: 1277, width: 41, height: 30),
        CGRect(x: 2801, y: 1308, width: 41, height: 30),
        CGRect(x: 2801, y: 1338, width: 41, height: 30),
        CGRect(x: 2801, y: 1369, width: 41, height: 30),
        CGRect(x: 2842, y: 943, width: 41, height: 30),
        CGRect(x: 2842, y: 974, width: 41, height: 30),
        CGRect(x: 2842, y: 1004, width: 41, height: 30),
        CGRect(x: 2842, y: 1034, width: 41, height: 30),
        CGRect(x: 2842, y: 1065, width: 41, height: 30),
        CGRect(x: 2842, y: 1095, width: 41, height: 30),
        CGRect(x: 2842, y: 1186, width: 41, height: 30),
        CGRect(x: 2842, y: 1217, width: 41, height: 30),
        CGRect(x: 2842, y: 1247, width: 41, height: 30),
        CGRect(x: 2842, y: 1277, width: 41, height: 30),
        CGRect(x: 2842, y: 1308, width: 41, height: 30),
        CGRect(x: 2842, y: 1338, width: 41, height: 30),
        CGRect(x: 2882, y: 974, width: 41, height: 30),
        CGRect(x: 2882, y: 1004, width: 41, height: 30),
        CGRect(x: 2882, y: 1034, width: 41, height: 30),
        CGRect(x: 2882, y: 1065, width: 41, height: 30),
        CGRect(x: 2882, y: 1095, width: 41, height: 30),
        CGRect(x: 2882, y: 1125, width: 41, height: 30),
        CGRect(x: 2882, y: 1156, width: 41, height: 30),
        CGRect(x: 2882, y: 1186, width: 41, height: 30),
        CGRect(x: 2882, y: 1217, width: 41, height: 30),
        CGRect(x: 2882, y: 1247, width: 41, height: 30),
        CGRect(x: 2882, y: 1277, width: 41, height: 30),
        CGRect(x: 2882, y: 1308, width: 41, height: 30),
        CGRect(x: 2923, y: 1004, width: 41, height: 30),
        CGRect(x: 2923, y: 1034, width: 41, height: 30),
        CGRect(x: 2923, y: 1065, width: 41, height: 30),
        CGRect(x: 2923, y: 1095, width: 41, height: 30),
        CGRect(x: 2923, y: 1125, width: 41, height: 30),
        CGRect(x: 2923, y: 1156, width: 41, height: 30),
        CGRect(x: 2923, y: 1186, width: 41, height: 30),
        CGRect(x: 2923, y: 1217, width: 41, height: 30),
        CGRect(x: 2923, y: 1247, width: 41, height: 30),
        CGRect(x: 2923, y: 1277, width: 41, height: 30),
        CGRect(x: 2963, y: 1034, width: 41, height: 30),
        CGRect(x: 2963, y: 1065, width: 41, height: 30),
        CGRect(x: 2963, y: 1095, width: 41, height: 30),
        CGRect(x: 2963, y: 1125, width: 41, height: 30),
        CGRect(x: 2963, y: 1156, width: 41, height: 30),
        CGRect(x: 2963, y: 1186, width: 41, height: 30),
        CGRect(x: 2963, y: 1217, width: 41, height: 30),
        CGRect(x: 2963, y: 1247, width: 41, height: 30),
    ]

    /// Partition solids: overlapping tight AABBs along the wall; doorway open.
    static let authoredPartitionWallNorthObstacle = CGRect(x: 1509, y: 1647, width: 40, height: 20)
    static let authoredPartitionWallSouthObstacle = CGRect(x: 2588, y: 817, width: 40, height: 20)
    static let authoredPartitionSegments: [CGRect] = [
        CGRect(x: 1509, y: 1647, width: 40, height: 20),
        CGRect(x: 1502, y: 1642, width: 40, height: 20),
        CGRect(x: 1495, y: 1637, width: 40, height: 20),
        CGRect(x: 1524, y: 1635, width: 40, height: 20),
        CGRect(x: 1518, y: 1630, width: 40, height: 20),
        CGRect(x: 1511, y: 1625, width: 40, height: 20),
        CGRect(x: 1540, y: 1624, width: 40, height: 20),
        CGRect(x: 1533, y: 1619, width: 40, height: 20),
        CGRect(x: 1526, y: 1614, width: 40, height: 20),
        CGRect(x: 1555, y: 1612, width: 40, height: 20),
        CGRect(x: 1548, y: 1607, width: 40, height: 20),
        CGRect(x: 1542, y: 1602, width: 40, height: 20),
        CGRect(x: 1571, y: 1601, width: 40, height: 20),
        CGRect(x: 1564, y: 1596, width: 40, height: 20),
        CGRect(x: 1557, y: 1591, width: 40, height: 20),
        CGRect(x: 1586, y: 1589, width: 40, height: 20),
        CGRect(x: 1579, y: 1584, width: 40, height: 20),
        CGRect(x: 1573, y: 1579, width: 40, height: 20),
        CGRect(x: 1601, y: 1578, width: 40, height: 20),
        CGRect(x: 1595, y: 1573, width: 40, height: 20),
        CGRect(x: 1588, y: 1567, width: 40, height: 20),
        CGRect(x: 1617, y: 1566, width: 40, height: 20),
        CGRect(x: 1610, y: 1561, width: 40, height: 20),
        CGRect(x: 1603, y: 1556, width: 40, height: 20),
        CGRect(x: 1632, y: 1554, width: 40, height: 20),
        CGRect(x: 1626, y: 1549, width: 40, height: 20),
        CGRect(x: 1619, y: 1544, width: 40, height: 20),
        CGRect(x: 1648, y: 1543, width: 40, height: 20),
        CGRect(x: 1641, y: 1538, width: 40, height: 20),
        CGRect(x: 1634, y: 1533, width: 40, height: 20),
        CGRect(x: 1663, y: 1531, width: 40, height: 20),
        CGRect(x: 1656, y: 1526, width: 40, height: 20),
        CGRect(x: 1650, y: 1521, width: 40, height: 20),
        CGRect(x: 1679, y: 1520, width: 40, height: 20),
        CGRect(x: 1672, y: 1515, width: 40, height: 20),
        CGRect(x: 1665, y: 1510, width: 40, height: 20),
        CGRect(x: 1694, y: 1508, width: 40, height: 20),
        CGRect(x: 1687, y: 1503, width: 40, height: 20),
        CGRect(x: 1681, y: 1498, width: 40, height: 20),
        CGRect(x: 1709, y: 1497, width: 40, height: 20),
        CGRect(x: 1703, y: 1491, width: 40, height: 20),
        CGRect(x: 1696, y: 1486, width: 40, height: 20),
        CGRect(x: 1725, y: 1485, width: 40, height: 20),
        CGRect(x: 1718, y: 1480, width: 40, height: 20),
        CGRect(x: 1711, y: 1475, width: 40, height: 20),
        CGRect(x: 1740, y: 1473, width: 40, height: 20),
        CGRect(x: 1734, y: 1468, width: 40, height: 20),
        CGRect(x: 1727, y: 1463, width: 40, height: 20),
        CGRect(x: 1756, y: 1462, width: 40, height: 20),
        CGRect(x: 1749, y: 1457, width: 40, height: 20),
        CGRect(x: 1742, y: 1452, width: 40, height: 20),
        CGRect(x: 1771, y: 1450, width: 40, height: 20),
        CGRect(x: 1764, y: 1445, width: 40, height: 20),
        CGRect(x: 1758, y: 1440, width: 40, height: 20),
        CGRect(x: 1787, y: 1439, width: 40, height: 20),
        CGRect(x: 1780, y: 1434, width: 40, height: 20),
        CGRect(x: 1773, y: 1429, width: 40, height: 20),
        CGRect(x: 1802, y: 1427, width: 40, height: 20),
        CGRect(x: 1795, y: 1422, width: 40, height: 20),
        CGRect(x: 1789, y: 1417, width: 40, height: 20),
        CGRect(x: 1817, y: 1415, width: 40, height: 20),
        CGRect(x: 1811, y: 1410, width: 40, height: 20),
        CGRect(x: 1804, y: 1405, width: 40, height: 20),
        CGRect(x: 1833, y: 1404, width: 40, height: 20),
        CGRect(x: 1826, y: 1399, width: 40, height: 20),
        CGRect(x: 1819, y: 1394, width: 40, height: 20),
        CGRect(x: 1848, y: 1392, width: 40, height: 20),
        CGRect(x: 1842, y: 1387, width: 40, height: 20),
        CGRect(x: 1835, y: 1382, width: 40, height: 20),
        CGRect(x: 1864, y: 1381, width: 40, height: 20),
        CGRect(x: 1857, y: 1376, width: 40, height: 20),
        CGRect(x: 1850, y: 1371, width: 40, height: 20),
        CGRect(x: 1879, y: 1369, width: 40, height: 20),
        CGRect(x: 1872, y: 1364, width: 40, height: 20),
        CGRect(x: 1866, y: 1359, width: 40, height: 20),
        CGRect(x: 1895, y: 1358, width: 40, height: 20),
        CGRect(x: 1888, y: 1353, width: 40, height: 20),
        CGRect(x: 1881, y: 1348, width: 40, height: 20),
        CGRect(x: 1910, y: 1346, width: 40, height: 20),
        CGRect(x: 1903, y: 1341, width: 40, height: 20),
        CGRect(x: 1897, y: 1336, width: 40, height: 20),
        CGRect(x: 2108, y: 1198, width: 40, height: 20),
        CGRect(x: 2101, y: 1193, width: 40, height: 20),
        CGRect(x: 2094, y: 1188, width: 40, height: 20),
        CGRect(x: 2123, y: 1186, width: 40, height: 20),
        CGRect(x: 2116, y: 1181, width: 40, height: 20),
        CGRect(x: 2110, y: 1176, width: 40, height: 20),
        CGRect(x: 2138, y: 1175, width: 40, height: 20),
        CGRect(x: 2132, y: 1170, width: 40, height: 20),
        CGRect(x: 2125, y: 1165, width: 40, height: 20),
        CGRect(x: 2154, y: 1163, width: 40, height: 20),
        CGRect(x: 2147, y: 1158, width: 40, height: 20),
        CGRect(x: 2140, y: 1153, width: 40, height: 20),
        CGRect(x: 2169, y: 1152, width: 40, height: 20),
        CGRect(x: 2163, y: 1147, width: 40, height: 20),
        CGRect(x: 2156, y: 1142, width: 40, height: 20),
        CGRect(x: 2185, y: 1140, width: 40, height: 20),
        CGRect(x: 2178, y: 1135, width: 40, height: 20),
        CGRect(x: 2171, y: 1130, width: 40, height: 20),
        CGRect(x: 2200, y: 1128, width: 40, height: 20),
        CGRect(x: 2193, y: 1123, width: 40, height: 20),
        CGRect(x: 2187, y: 1118, width: 40, height: 20),
        CGRect(x: 2216, y: 1117, width: 40, height: 20),
        CGRect(x: 2209, y: 1112, width: 40, height: 20),
        CGRect(x: 2202, y: 1107, width: 40, height: 20),
        CGRect(x: 2231, y: 1105, width: 40, height: 20),
        CGRect(x: 2224, y: 1100, width: 40, height: 20),
        CGRect(x: 2218, y: 1095, width: 40, height: 20),
        CGRect(x: 2246, y: 1094, width: 40, height: 20),
        CGRect(x: 2240, y: 1089, width: 40, height: 20),
        CGRect(x: 2233, y: 1084, width: 40, height: 20),
        CGRect(x: 2262, y: 1082, width: 40, height: 20),
        CGRect(x: 2255, y: 1077, width: 40, height: 20),
        CGRect(x: 2249, y: 1072, width: 40, height: 20),
        CGRect(x: 2277, y: 1071, width: 40, height: 20),
        CGRect(x: 2271, y: 1066, width: 40, height: 20),
        CGRect(x: 2264, y: 1061, width: 40, height: 20),
        CGRect(x: 2293, y: 1059, width: 40, height: 20),
        CGRect(x: 2286, y: 1054, width: 40, height: 20),
        CGRect(x: 2279, y: 1049, width: 40, height: 20),
        CGRect(x: 2308, y: 1047, width: 40, height: 20),
        CGRect(x: 2301, y: 1042, width: 40, height: 20),
        CGRect(x: 2295, y: 1037, width: 40, height: 20),
        CGRect(x: 2324, y: 1036, width: 40, height: 20),
        CGRect(x: 2317, y: 1031, width: 40, height: 20),
        CGRect(x: 2310, y: 1026, width: 40, height: 20),
        CGRect(x: 2339, y: 1024, width: 40, height: 20),
        CGRect(x: 2332, y: 1019, width: 40, height: 20),
        CGRect(x: 2326, y: 1014, width: 40, height: 20),
        CGRect(x: 2354, y: 1013, width: 40, height: 20),
        CGRect(x: 2348, y: 1008, width: 40, height: 20),
        CGRect(x: 2341, y: 1003, width: 40, height: 20),
        CGRect(x: 2370, y: 1001, width: 40, height: 20),
        CGRect(x: 2363, y: 996, width: 40, height: 20),
        CGRect(x: 2357, y: 991, width: 40, height: 20),
        CGRect(x: 2385, y: 990, width: 40, height: 20),
        CGRect(x: 2379, y: 985, width: 40, height: 20),
        CGRect(x: 2372, y: 980, width: 40, height: 20),
        CGRect(x: 2401, y: 978, width: 40, height: 20),
        CGRect(x: 2394, y: 973, width: 40, height: 20),
        CGRect(x: 2387, y: 968, width: 40, height: 20),
        CGRect(x: 2416, y: 966, width: 40, height: 20),
        CGRect(x: 2410, y: 961, width: 40, height: 20),
        CGRect(x: 2403, y: 956, width: 40, height: 20),
        CGRect(x: 2432, y: 955, width: 40, height: 20),
        CGRect(x: 2425, y: 950, width: 40, height: 20),
        CGRect(x: 2418, y: 945, width: 40, height: 20),
        CGRect(x: 2447, y: 943, width: 40, height: 20),
        CGRect(x: 2440, y: 938, width: 40, height: 20),
        CGRect(x: 2434, y: 933, width: 40, height: 20),
        CGRect(x: 2463, y: 932, width: 40, height: 20),
        CGRect(x: 2456, y: 927, width: 40, height: 20),
        CGRect(x: 2449, y: 922, width: 40, height: 20),
        CGRect(x: 2478, y: 920, width: 40, height: 20),
        CGRect(x: 2471, y: 915, width: 40, height: 20),
        CGRect(x: 2465, y: 910, width: 40, height: 20),
        CGRect(x: 2493, y: 909, width: 40, height: 20),
        CGRect(x: 2487, y: 904, width: 40, height: 20),
        CGRect(x: 2480, y: 899, width: 40, height: 20),
        CGRect(x: 2509, y: 897, width: 40, height: 20),
        CGRect(x: 2502, y: 892, width: 40, height: 20),
        CGRect(x: 2495, y: 887, width: 40, height: 20),
        CGRect(x: 2524, y: 885, width: 40, height: 20),
        CGRect(x: 2518, y: 880, width: 40, height: 20),
        CGRect(x: 2511, y: 875, width: 40, height: 20),
        CGRect(x: 2540, y: 874, width: 40, height: 20),
        CGRect(x: 2533, y: 869, width: 40, height: 20),
        CGRect(x: 2526, y: 864, width: 40, height: 20),
        CGRect(x: 2555, y: 862, width: 40, height: 20),
        CGRect(x: 2548, y: 857, width: 40, height: 20),
        CGRect(x: 2542, y: 852, width: 40, height: 20),
        CGRect(x: 2571, y: 851, width: 40, height: 20),
        CGRect(x: 2564, y: 846, width: 40, height: 20),
        CGRect(x: 2557, y: 841, width: 40, height: 20),
        CGRect(x: 2586, y: 839, width: 40, height: 20),
        CGRect(x: 2579, y: 834, width: 40, height: 20),
        CGRect(x: 2573, y: 829, width: 40, height: 20),
        CGRect(x: 2601, y: 828, width: 40, height: 20),
        CGRect(x: 2595, y: 823, width: 40, height: 20),
        CGRect(x: 2588, y: 817, width: 40, height: 20),
    ]

    static let authoredSafeObstacle = CGRect(x: 1405, y: 1443, width: 88, height: 66)
    static let authoredFilingCabinetBObstacle = CGRect(x: 1348, y: 1401, width: 80, height: 60)
    static let authoredFilingCabinetObstacle = CGRect(x: 1292, y: 1359, width: 80, height: 60)
    static let authoredBookshelfObstacle = CGRect(x: 1199, y: 1289, width: 127, height: 96)
    static let authoredArchiveBoxAObstacle = CGRect(x: 1311, y: 1293, width: 70, height: 53)
    static let authoredRadiatorObstacle = CGRect(x: 1237, y: 1327, width: 101, height: 76)
    static let authoredPersonalSideboardObstacle = CGRect(x: 1545, y: 1548, width: 136, height: 102)
    static let authoredPersonalFanObstacle = CGRect(x: 1626, y: 1560, width: 73, height: 55)
    static let authoredDeskEnsembleObstacle = CGRect(x: 1544, y: 1252, width: 203, height: 153)
    static let authoredVisitorArmchairObstacle = CGRect(x: 1640, y: 1417, width: 95, height: 71)
    static let authoredVisitorArmchairBObstacle = CGRect(x: 1743, y: 1339, width: 95, height: 71)
    static let authoredWastebasketObstacle = CGRect(x: 1474, y: 1323, width: 58, height: 44)
    static let authoredCoatRackObstacle = CGRect(x: 2761, y: 1177, width: 88, height: 66)
    static let authoredUmbrellaStandObstacle = CGRect(x: 2712, y: 1202, width: 51, height: 38)
    static let authoredWaitingChairAObstacle = CGRect(x: 2344, y: 1327, width: 80, height: 60)
    static let authoredWaitingTableObstacle = CGRect(x: 2406, y: 1281, width: 80, height: 60)
    static let authoredWaitingChairBObstacle = CGRect(x: 2468, y: 1234, width: 80, height: 60)

    private static var authoredObstacles: [CGRect] {
        [authoredDoorObstacle, authoredForegroundWallObstacle]
            + authoredBoundarySegments
            + authoredPartitionSegments
            + [
                authoredSafeObstacle,
                authoredFilingCabinetBObstacle,
                authoredFilingCabinetObstacle,
                authoredBookshelfObstacle,
                authoredArchiveBoxAObstacle,
                authoredRadiatorObstacle,
                authoredPersonalSideboardObstacle,
                authoredPersonalFanObstacle,
                authoredDeskEnsembleObstacle,
                authoredVisitorArmchairObstacle,
                authoredVisitorArmchairBObstacle,
                authoredWastebasketObstacle,
                authoredCoatRackObstacle,
                authoredUmbrellaStandObstacle,
                authoredWaitingChairAObstacle,
                authoredWaitingTableObstacle,
                authoredWaitingChairBObstacle,
            ]
    }

    // MARK: - Sample points (interior to each obstacle)

    static let authoredSafeSamplePoints: [CGPoint] = [
        CGPoint(x: 1_448, y: 1_476),
        CGPoint(x: 1_431, y: 1_467),
        CGPoint(x: 1_466, y: 1_486),
    ]
    static let authoredFilingCabinetBSamplePoints: [CGPoint] = [
        CGPoint(x: 1_388, y: 1_431),
        CGPoint(x: 1_372, y: 1_422),
        CGPoint(x: 1_404, y: 1_440),
    ]
    static let authoredFilingCabinetSamplePoints: [CGPoint] = [
        CGPoint(x: 1_332, y: 1_389),
        CGPoint(x: 1_316, y: 1_380),
        CGPoint(x: 1_348, y: 1_398),
    ]
    static let authoredBookshelfSamplePoints: [CGPoint] = [
        CGPoint(x: 1_263, y: 1_337),
        CGPoint(x: 1_237, y: 1_323),
        CGPoint(x: 1_288, y: 1_352),
    ]
    static let authoredArchiveBoxASamplePoints: [CGPoint] = [
        CGPoint(x: 1_347, y: 1_320),
        CGPoint(x: 1_333, y: 1_312),
        CGPoint(x: 1_361, y: 1_328),
    ]
    static let authoredRadiatorSamplePoints: [CGPoint] = [
        CGPoint(x: 1_288, y: 1_365),
        CGPoint(x: 1_268, y: 1_354),
        CGPoint(x: 1_308, y: 1_376),
    ]
    static let authoredPersonalSideboardSamplePoints: [CGPoint] = [
        CGPoint(x: 1_613, y: 1_599),
        CGPoint(x: 1_585, y: 1_584),
        CGPoint(x: 1_640, y: 1_615),
    ]
    static let authoredPersonalFanSamplePoints: [CGPoint] = [
        CGPoint(x: 1_663, y: 1_588),
        CGPoint(x: 1_648, y: 1_579),
        CGPoint(x: 1_677, y: 1_596),
    ]
    static let authoredDeskEnsembleSamplePoints: [CGPoint] = [
        CGPoint(x: 1_646, y: 1_328),
        CGPoint(x: 1_605, y: 1_305),
        CGPoint(x: 1_687, y: 1_351),
    ]
    static let authoredVisitorArmchairSamplePoints: [CGPoint] = [
        CGPoint(x: 1_688, y: 1_452),
        CGPoint(x: 1_669, y: 1_442),
        CGPoint(x: 1_707, y: 1_463),
    ]
    static let authoredVisitorArmchairBSamplePoints: [CGPoint] = [
        CGPoint(x: 1_791, y: 1_375),
        CGPoint(x: 1_772, y: 1_364),
        CGPoint(x: 1_810, y: 1_386),
    ]
    static let authoredWastebasketSamplePoints: [CGPoint] = [
        CGPoint(x: 1_503, y: 1_345),
        CGPoint(x: 1_492, y: 1_338),
        CGPoint(x: 1_515, y: 1_351),
    ]
    static let authoredCoatRackSamplePoints: [CGPoint] = [
        CGPoint(x: 2_805, y: 1_210),
        CGPoint(x: 2_788, y: 1_200),
        CGPoint(x: 2_823, y: 1_220),
    ]
    static let authoredUmbrellaStandSamplePoints: [CGPoint] = [
        CGPoint(x: 2_738, y: 1_221),
        CGPoint(x: 2_728, y: 1_216),
        CGPoint(x: 2_748, y: 1_227),
    ]
    static let authoredWaitingChairASamplePoints: [CGPoint] = [
        CGPoint(x: 2_384, y: 1_357),
        CGPoint(x: 2_368, y: 1_348),
        CGPoint(x: 2_400, y: 1_366),
    ]
    static let authoredWaitingTableSamplePoints: [CGPoint] = [
        CGPoint(x: 2_446, y: 1_311),
        CGPoint(x: 2_430, y: 1_302),
        CGPoint(x: 2_462, y: 1_320),
    ]
    static let authoredWaitingChairBSamplePoints: [CGPoint] = [
        CGPoint(x: 2_508, y: 1_265),
        CGPoint(x: 2_492, y: 1_256),
        CGPoint(x: 2_524, y: 1_274),
    ]
    static let authoredDoorLeafSamplePoints: [CGPoint] = [
        CGPoint(x: 2_822, y: 1_249),
        CGPoint(x: 2_780, y: 1_228),
        CGPoint(x: 2_865, y: 1_270),
    ]
    static let authoredForegroundWallSamplePoints: [CGPoint] = [
        CGPoint(x: 2_554, y: 867),
        CGPoint(x: 2_533, y: 856),
        CGPoint(x: 2_575, y: 879),
    ]
    static let authoredPartitionWallNorthSamplePoints: [CGPoint] = [
        CGPoint(x: 1_529, y: 1_657),
        CGPoint(x: 1_521, y: 1_654),
        CGPoint(x: 1_537, y: 1_660),
    ]
    static let authoredPartitionWallSouthSamplePoints: [CGPoint] = [
        CGPoint(x: 2_608, y: 827),
        CGPoint(x: 2_600, y: 824),
        CGPoint(x: 2_616, y: 830),
    ]

    // MARK: - Placements

    enum AuthoredPlacement {
        /// Registered to the visual threshold baked into the shipping suite.
        static let doorLeaf = Architecture.entranceLeafAnchor
        static let window = Architecture.windowAnchor
        static let windowBlinds = window
        static let windowRotation: CGFloat = 0
        static let windowRainMask = CGRect(x: 1_222.2, y: 1_599.2, width: 45.6, height: 81.6)
        static let windowRainEmitter = CGPoint(x: 1_245.0, y: 1_683.2)
        /// Recentred on the fitted cramped room diamond.
        static let camera = CGPoint(x: 1_918, y: 1_254)

        static let safe = CGPoint(x: 1_448, y: 1_476)  // records run, east end
        static let filingCabinetB = CGPoint(x: 1_388, y: 1_431)
        static let filingCabinet = CGPoint(x: 1_332, y: 1_389)  // drawer half open
        static let bookshelf = CGPoint(x: 1_263, y: 1_337)
        static let archiveBoxOnCabinet = CGPoint(x: 1_388, y: 1_431)  // on cabinet B
        static let archiveStackOnCabinet = CGPoint(x: 1_332, y: 1_389)  // on cabinet A
        static let archiveBoxA = CGPoint(x: 1_347, y: 1_320)  // only floor stack
        static let radiator = CGPoint(x: 1_288, y: 1_365)
        static let personalSideboard = CGPoint(x: 1_613, y: 1_599)
        static let personalWashbasin = CGPoint(x: 1_565, y: 1_564)  // retired domestic fixture; placement retained for source lineage
        static let personalFan = CGPoint(x: 1_663, y: 1_588)
        static let personalBottle = CGPoint(x: 1_610, y: 1_591)  // on sideboard
        static let personalGlass = CGPoint(x: 1_625, y: 1_603)  // on sideboard
        static let deskEnsemble = CGPoint(x: 1_646, y: 1_328)
        static let deskChair = CGPoint(x: 1_581, y: 1_280)
        static let visitorArmchair = CGPoint(x: 1_688, y: 1_452)
        static let visitorArmchairB = CGPoint(x: 1_791, y: 1_375)
        static let wastebasket = CGPoint(x: 1_503, y: 1_345)
        static let coatRack = CGPoint(x: 2_805, y: 1_210)
        static let umbrellaStand = CGPoint(x: 2_738, y: 1_221)
        static let waitingChairA = CGPoint(x: 2_384, y: 1_357)
        static let waitingTable = CGPoint(x: 2_446, y: 1_311)
        static let waitingChairB = CGPoint(x: 2_508, y: 1_265)
        static let newspaper = CGPoint(x: 2_445, y: 1_319)  // on table
        static let waitingAshtray = CGPoint(x: 2_447, y: 1_300)  // on table

        static let wornRug = CGPoint(x: 1_611, y: 1_302)
        static let floorWear = deskEnsemble
        static let wallPhotos = CGPoint(x: 1_051, y: 1_564)
        static let caseBoard = CGPoint(x: 1_190, y: 1_783)
        static let wallCityMap = CGPoint(x: 1_379, y: 1_877)
        static let framedLicence = CGPoint(x: 1_535, y: 1_895)
        static let windowSpill = CGPoint(x: 1_347, y: 1_320)
        static let blindStripes = CGPoint(x: 1_375, y: 1_248)
        static let hallwayLight = CGPoint(x: 2_805, y: 1_236)
        static let floorTrashA = CGPoint(x: 1_680, y: 1_277)
        static let floorTrashB = CGPoint(x: 1_386, y: 1_303)
        static let entranceRunner = CGPoint(x: 2_693, y: 1_249)
        static let lampPool = deskEnsemble
        /// Leaf swung 90° into the private office, hinged on the up-run jamb.
        static let internalDoorLeaf = Architecture.internalLeafAnchor
    }

    private static let authoredApproachPoints: [String: CGPoint] = [
        "office.window": CGPoint(x: 1_519, y: 1_294),
        "office.desk": CGPoint(x: 1_856, y: 1_300),
        "office.phone": CGPoint(x: 1_839, y: 1_287),
        "office.files": CGPoint(x: 1_447, y: 1_303),
        "office.door": CGPoint(x: 2_386, y: 1_420),
    ]

    private static let authoredProjectionOrigin = CGPoint(x: 2_048, y: 518)
    private static let authoredTileSize = CGSize(width: 128, height: 96)

    static var actorStart: CGPoint { OfficeInteriorScale.mapPoint(authoredActorStart) }

    static var emptyDeskChairWorldPosition: CGPoint {
        let baseline = OfficeInteriorScale.mapPoint(AuthoredPlacement.deskChair)
        let nudge = OfficeInteriorScale.ActorDisplay.seatedDeskNudge
        return CGPoint(x: baseline.x + nudge.x, y: baseline.y + nudge.y)
    }

    enum DeskDepth {
        /// Visitor chairs sit on the far side of the writing surface.
        static let visitorChairBias: CGFloat = -50
        static let topOccluderBias: CGFloat = -40
        /// Above seated Voss's lower layer, below a client passing camera-near.
        static let seatedFrontApronBias: CGFloat = 15
        static let standingFrontApronBias: CGFloat = 40
    }

    /// Exterior threshold crossing. This segment intentionally starts
    /// outside the navigation floor and passes through the fallen door leaf.
    static let clientDoorwayPath: [CGPoint] = [
        CGPoint(x: 2_891, y: 1_301),
        CGPoint(x: 2_650, y: 1_119),
    ].map(OfficeInteriorScale.mapPoint)

    /// Walkable waiting-room anchors, ending immediately before the
    /// framed internal doorway in the production suite plate.
    static let clientWaitingRoomPath: [CGPoint] = [
        CGPoint(x: 2_650, y: 1_119),
        CGPoint(x: 2_236, y: 1_339),
        CGPoint(x: 2_169, y: 1_389),
        CGPoint(x: 2_095, y: 1_333),
    ].map(OfficeInteriorScale.mapPoint)

    /// Explicit crossing through the real framed partition aperture.
    static let clientInternalDoorwayPath: [CGPoint] = [
        CGPoint(x: 2_095, y: 1_333),
        CGPoint(x: 2_021, y: 1_278),
        CGPoint(x: 1_956, y: 1_229),
    ].map(OfficeInteriorScale.mapPoint)

    /// Direct private-office approach after clearing the internal door:
    /// one step along the aperture b to clear the jamb, then on to the
    /// desk's camera-near corner where the visitor stands to talk.
    static let clientOfficeArrivalPath: [CGPoint] = [
        CGPoint(x: 1_956, y: 1_229),
        CGPoint(x: 1_919, y: 1_201),
        CGPoint(x: 1_856, y: 1_300),
    ].map(OfficeInteriorScale.mapPoint)

    static let clientInteriorArrivalPath: [CGPoint] = [
        clientWaitingRoomPath,
        Array(clientInternalDoorwayPath.dropFirst()),
        Array(clientOfficeArrivalPath.dropFirst()),
    ].flatMap { $0 }

    static var clientArrivalPath: [CGPoint] {
        Array(clientDoorwayPath.dropLast()) + clientInteriorArrivalPath
    }

    static var clientDeparturePath: [CGPoint] { Array(clientArrivalPath.reversed()) }

    /// Authored aperture polyline through the shipping painted doorway.
    /// Do not A*-expand: snapping interior anchors onto nearest walkable
    /// cells can walk the coat through frosted glass beside the real opening.
    /// Anchors are collision-checked at layout generation.
    static func clientArrivalRoute(in navigation: NavigationMap) -> [CGPoint] {
        _ = navigation
        return clientArrivalPath
    }

    static func clientDepartureRoute(in navigation: NavigationMap) -> [CGPoint] {
        Array(clientArrivalRoute(in: navigation).reversed())
    }

    static let exteriorToInternalDoorPath: [CGPoint] = [
        Array(clientDoorwayPath.dropLast()),
        clientWaitingRoomPath,
        Array(clientInternalDoorwayPath.dropFirst()),
    ].flatMap { $0 }

    static let internalDoorToClientPath: [CGPoint] = [
        clientInternalDoorwayPath,
        Array(clientOfficeArrivalPath.dropFirst()),
    ].flatMap { $0 }

    static let recordsApproachPath: [CGPoint] = [
        CGPoint(x: 1_673, y: 1_256),
        CGPoint(x: 1_519, y: 1_294),
        CGPoint(x: 1_447, y: 1_303),
    ].map(OfficeInteriorScale.mapPoint)

    static var obstacles: [CGRect] { authoredObstacles.map(OfficeInteriorScale.mapRect) }

    static var safeObstacle: CGRect { OfficeInteriorScale.mapRect(authoredSafeObstacle) }
    static var filingCabinetBObstacle: CGRect { OfficeInteriorScale.mapRect(authoredFilingCabinetBObstacle) }
    static var filingCabinetObstacle: CGRect { OfficeInteriorScale.mapRect(authoredFilingCabinetObstacle) }
    static var bookshelfObstacle: CGRect { OfficeInteriorScale.mapRect(authoredBookshelfObstacle) }
    static var archiveBoxAObstacle: CGRect { OfficeInteriorScale.mapRect(authoredArchiveBoxAObstacle) }
    static var radiatorObstacle: CGRect { OfficeInteriorScale.mapRect(authoredRadiatorObstacle) }
    static var personalSideboardObstacle: CGRect { OfficeInteriorScale.mapRect(authoredPersonalSideboardObstacle) }
    static var personalFanObstacle: CGRect { OfficeInteriorScale.mapRect(authoredPersonalFanObstacle) }
    static var deskEnsembleObstacle: CGRect { OfficeInteriorScale.mapRect(authoredDeskEnsembleObstacle) }
    static var visitorArmchairObstacle: CGRect { OfficeInteriorScale.mapRect(authoredVisitorArmchairObstacle) }
    static var visitorArmchairBObstacle: CGRect { OfficeInteriorScale.mapRect(authoredVisitorArmchairBObstacle) }
    static var wastebasketObstacle: CGRect { OfficeInteriorScale.mapRect(authoredWastebasketObstacle) }
    static var coatRackObstacle: CGRect { OfficeInteriorScale.mapRect(authoredCoatRackObstacle) }
    static var umbrellaStandObstacle: CGRect { OfficeInteriorScale.mapRect(authoredUmbrellaStandObstacle) }
    static var waitingChairAObstacle: CGRect { OfficeInteriorScale.mapRect(authoredWaitingChairAObstacle) }
    static var waitingTableObstacle: CGRect { OfficeInteriorScale.mapRect(authoredWaitingTableObstacle) }
    static var waitingChairBObstacle: CGRect { OfficeInteriorScale.mapRect(authoredWaitingChairBObstacle) }
    static var doorObstacle: CGRect { OfficeInteriorScale.mapRect(authoredDoorObstacle) }
    static var foregroundWallObstacle: CGRect { OfficeInteriorScale.mapRect(authoredForegroundWallObstacle) }
    static var partitionWallNorthObstacle: CGRect { OfficeInteriorScale.mapRect(authoredPartitionWallNorthObstacle) }
    static var partitionWallSouthObstacle: CGRect { OfficeInteriorScale.mapRect(authoredPartitionWallSouthObstacle) }

    static var safeSamplePoints: [CGPoint] { authoredSafeSamplePoints.map(OfficeInteriorScale.mapPoint) }
    static var filingCabinetBSamplePoints: [CGPoint] { authoredFilingCabinetBSamplePoints.map(OfficeInteriorScale.mapPoint) }
    static var filingCabinetSamplePoints: [CGPoint] { authoredFilingCabinetSamplePoints.map(OfficeInteriorScale.mapPoint) }
    static var bookshelfSamplePoints: [CGPoint] { authoredBookshelfSamplePoints.map(OfficeInteriorScale.mapPoint) }
    static var archiveBoxASamplePoints: [CGPoint] { authoredArchiveBoxASamplePoints.map(OfficeInteriorScale.mapPoint) }
    static var radiatorSamplePoints: [CGPoint] { authoredRadiatorSamplePoints.map(OfficeInteriorScale.mapPoint) }
    static var personalSideboardSamplePoints: [CGPoint] { authoredPersonalSideboardSamplePoints.map(OfficeInteriorScale.mapPoint) }
    static var personalFanSamplePoints: [CGPoint] { authoredPersonalFanSamplePoints.map(OfficeInteriorScale.mapPoint) }
    static var deskEnsembleSamplePoints: [CGPoint] { authoredDeskEnsembleSamplePoints.map(OfficeInteriorScale.mapPoint) }
    static var visitorArmchairSamplePoints: [CGPoint] { authoredVisitorArmchairSamplePoints.map(OfficeInteriorScale.mapPoint) }
    static var visitorArmchairBSamplePoints: [CGPoint] { authoredVisitorArmchairBSamplePoints.map(OfficeInteriorScale.mapPoint) }
    static var wastebasketSamplePoints: [CGPoint] { authoredWastebasketSamplePoints.map(OfficeInteriorScale.mapPoint) }
    static var coatRackSamplePoints: [CGPoint] { authoredCoatRackSamplePoints.map(OfficeInteriorScale.mapPoint) }
    static var umbrellaStandSamplePoints: [CGPoint] { authoredUmbrellaStandSamplePoints.map(OfficeInteriorScale.mapPoint) }
    static var waitingChairASamplePoints: [CGPoint] { authoredWaitingChairASamplePoints.map(OfficeInteriorScale.mapPoint) }
    static var waitingTableSamplePoints: [CGPoint] { authoredWaitingTableSamplePoints.map(OfficeInteriorScale.mapPoint) }
    static var waitingChairBSamplePoints: [CGPoint] { authoredWaitingChairBSamplePoints.map(OfficeInteriorScale.mapPoint) }
    static var doorLeafSamplePoints: [CGPoint] { authoredDoorLeafSamplePoints.map(OfficeInteriorScale.mapPoint) }
    static var foregroundWallSamplePoints: [CGPoint] { authoredForegroundWallSamplePoints.map(OfficeInteriorScale.mapPoint) }
    static var partitionWallNorthSamplePoints: [CGPoint] { authoredPartitionWallNorthSamplePoints.map(OfficeInteriorScale.mapPoint) }
    static var partitionWallSouthSamplePoints: [CGPoint] { authoredPartitionWallSouthSamplePoints.map(OfficeInteriorScale.mapPoint) }

    static var majorPropSamplePoints: [CGPoint] {
        safeSamplePoints
            + filingCabinetBSamplePoints
            + filingCabinetSamplePoints
            + bookshelfSamplePoints
            + archiveBoxASamplePoints
            + radiatorSamplePoints
            + personalSideboardSamplePoints
            + personalFanSamplePoints
            + deskEnsembleSamplePoints
            + visitorArmchairSamplePoints
            + visitorArmchairBSamplePoints
            + wastebasketSamplePoints
            + coatRackSamplePoints
            + umbrellaStandSamplePoints
            + waitingChairASamplePoints
            + waitingTableSamplePoints
            + waitingChairBSamplePoints
            + doorLeafSamplePoints
            + foregroundWallSamplePoints
            + partitionWallNorthSamplePoints
            + partitionWallSouthSamplePoints
    }

    static var approachPoints: [String: CGPoint] {
        authoredApproachPoints.mapValues(OfficeInteriorScale.mapPoint)
    }

    /// Actual Voss stand-ins for architecture scale review: exterior door,
    /// desk, internal doorway, and waiting chair.
    static let authoredScaleReferenceStands: [CGPoint] = [
        CGPoint(x: 2_753, y: 1_197),
        CGPoint(x: 1_577, y: 1_276),
        CGPoint(x: 2_057, y: 1_305),
        CGPoint(x: 2_405, y: 1_342),
    ]

    static var scaleReferenceStands: [CGPoint] {
        authoredScaleReferenceStands.map(OfficeInteriorScale.mapPoint)
    }

    static var deskSamplePoints: [CGPoint] { deskEnsembleSamplePoints }
    static var deskObstacle: CGRect { deskEnsembleObstacle }
    static var authoredDeskObstacle: CGRect { authoredDeskEnsembleObstacle }

    static let authoredHotspots: [(id: String, name: String, hitArea: CGRect, observation: String)] = [
        (
            "office.window",
            "Rain-streaked window",
            CGRect(x: 1_174, y: 1_758, width: 92, height: 160),
            "The rain had been working the glass harder than I had worked a case."
        ),
        (
            "office.desk",
            "Desk",
            deskHitArea,
            "Three old cases, two unpaid bills, one clean page."
        ),
        (
            "office.phone",
            "Telephone",
            phoneHitArea,
            "Quiet. For once it had the decency to look guilty."
        ),
        (
            "office.files",
            "Case files",
            filesHitArea,
            "Closed, abandoned, and one I still lied about."
        ),
        (
            "office.door",
            "Office door",
            doorHitArea,
            "The hall smelled worse, but at least it led somewhere."
        )
    ]

    /// BG-style authored loot for searchable office containers (resolve-once on area entry).
    static let lootContainers: [LootContainerDefinition] = [
        LootContainerDefinition(
            id: "office.desk",
            entries: [
                .coins(pence: 36), // 3s loose change in the drawer
                .randomCoins(table: RandomCoinTable(penceForRolls2to20: [
                    1, 2, 3, 4, 6, 6, 8, 8, 12, 12, 12, 18, 18, 24, 24, 30, 36, 36, 48
                ]))
            ]
        ),
        LootContainerDefinition(
            id: "office.files",
            entries: [
                .randomCoins(table: RandomCoinTable(penceForRolls2to20: [
                    1, 1, 2, 2, 3, 3, 4, 4, 6, 6, 6, 8, 8, 12, 12, 12, 18, 18, 24
                ]))
            ]
        )
    ]

    static func lootContainer(for hotspotID: String) -> LootContainerDefinition? {
        lootContainers.first { $0.id == hotspotID }
    }

    /// Tall aperture covering the entrance leaf and threshold obstacle.
    private static var doorHitArea: CGRect {
        CGRect(x: 2_600, y: 1_100, width: 293, height: 586)
    }

    private static var deskHitArea: CGRect {
        authoredDeskEnsembleObstacle.insetBy(dx: -90, dy: -70)
    }

    private static var phoneHitArea: CGRect {
        CGRect(
            x: AuthoredPlacement.deskEnsemble.x - 40,
            y: AuthoredPlacement.deskEnsemble.y + 40,
            width: 200,
            height: 150
        )
    }

    private static var filesHitArea: CGRect {
        CGRect(
            x: AuthoredPlacement.filingCabinet.x - 110,
            y: AuthoredPlacement.filingCabinet.y - 20,
            width: 300,
            height: 220
        )
    }


    enum DialogueCameraFraming {
        static let legacyDownwardOffset: CGFloat = 55
        static let priorDownwardOffset: CGFloat = 28
        /// Scaled with the BG:EE mid-band camera (~9% rendered body) so dialogue
        /// framing keeps the prior on-screen downward bias after the zoom-out.
        static let cameraBelowActorMidpoint: CGFloat = 142
        static let lateralBiasTowardClient: CGFloat = 24

        static var actorFocusPoint: CGPoint {
            let voss = OfficeInteriorScale.mapPoint(AuthoredPlacement.deskChair)
            let lila = clientArrivalPath.last
                ?? OfficeInteriorScale.mapPoint(AuthoredPlacement.visitorArmchair)
            return CGPoint(
                x: (voss.x + lila.x) * 0.5 + lateralBiasTowardClient,
                y: (voss.y + lila.y) * 0.5
            )
        }

        static var dialogueCameraWorldPosition: CGPoint {
            let focus = actorFocusPoint
            return CGPoint(x: focus.x, y: focus.y - cameraBelowActorMidpoint)
        }

        static func dialogueCameraPosition(playCamera: CGPoint) -> CGPoint {
            _ = playCamera
            return dialogueCameraWorldPosition
        }

        static var downwardOffsetFromPlayCamera: CGFloat {
            let play = OfficeInteriorScale.mapPoint(AuthoredPlacement.camera)
            return play.y - dialogueCameraWorldPosition.y
        }

        static var lateralOffsetFromPlayCamera: CGFloat {
            let play = OfficeInteriorScale.mapPoint(AuthoredPlacement.camera)
            return dialogueCameraWorldPosition.x - play.x
        }
    }

    /// World-space bounds of the scaled office plate (BG search-map frame).
    static var navigationWorldBounds: CGRect {
        CGRect(origin: OfficeInteriorScale.shellOrigin, size: OfficeInteriorScale.scaledArtSize)
    }

    /// - Parameter entranceDoorBlocking: When false, exterior door cells are
    ///   clear (door has fallen / opening is open). Toggle later via
    ///   `NavigationMap.setEntranceDoorBlocking` without rebuilding.
    static func makeGrid(entranceDoorBlocking: Bool = true) -> NavigationMap {
        NavigationMap(
            worldBounds: navigationWorldBounds,
            obstacles: obstacles,
            agentProfile: .officeDetective,
            doorObstacles: [doorObstacle],
            entranceDoorBlocking: entranceDoorBlocking,
            maxNodes: 96_000
        )
    }

    static func isBlocked(_ point: CGPoint) -> Bool {
        obstacles.contains { $0.contains(point) }
    }
}
