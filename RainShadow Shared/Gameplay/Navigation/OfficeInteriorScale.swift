...
enum OfficeInteriorScale {
    ...
    enum ActorDisplay {
        ...
        /// V13's indexed craft plane, measured crown-to-ground. The registered
        /// source body remains 200px so it keeps the established world size;
        /// the camera maps these 64 native rows to 64 view points at 100%.
        static let nativeStandingBodyPixelHeight: CGFloat = 64
        ...
        /// Whole-body registration from the authored chair ground point to the
        /// visual centre of its baked seat. The V22 NE body already contains the
        /// desk-facing lean, so the older positive-Y kneehole offset left Voss
        /// floating above and to the right of the chair.
        static let seatedDeskNudge = CGPoint(x: -20, y: 0)
        /// V22 owns the complete seated pose; no additional upper-body reach is
        /// needed after the whole body has been registered to the chair.
    ...
    /// V13's 64-row body native-sized on screen. Window size changes framing,
    /// not sprite magnification.
    static var cameraScaleAt100Percent: CGFloat {
        DefaultPlayZoom.nativeSpriteCameraScale(
            standingBodyHeight: renderedStandingDetectiveBodyHeight,
            nativeBodyPixelHeight: ActorDisplay.nativeStandingBodyPixelHeight
        )
    }
    ...
    static func cameraVisibleHeight(forSceneHeight sceneHeight: CGFloat) -> CGFloat {
        DefaultPlayZoom.cameraVisibleHeight(
            ...
            cameraScale: cameraScaleAt100Percent
        )
    }
    ...
    enum Band {
        ...
        /// The V20 affine stretches the plate vertically (~1.13x), so the
        /// warped panes measure up to 0.35 of an adult.
        static let windowPane: ClosedRange<CGFloat> = 0.22...0.36
    }
