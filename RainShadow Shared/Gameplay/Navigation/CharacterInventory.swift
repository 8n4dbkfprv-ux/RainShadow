...
struct CharacterInventory: Equatable, Sendable {
    ...
    /// How many of this item the character has, worn or in the bag.
    func quantity(of itemID: String) -> Int {
        let worn = equipped.values.reduce(0) { $0 + ($1.id == itemID ? $1.quantity : 0) }
        let carried = backpack.stacks.reduce(0) { $0 + ($1.id == itemID ? $1.quantity : 0) }
        return worn + carried
    }
