...
struct MovementProfile: Equatable, Sendable {
    ...
    /// Projected world units per second, through the same five engine constants
    /// `ActorLocomotionPacing` documents. Zero when immobile, which
    /// `RouteFollower.advance` already treats as "hold position".
    var walkSpeed: CGFloat {
        ...
        let bodyHeightsPerSecond = ActorLocomotionPacing.stepRadius
            * stepFactor
            * ActorLocomotionPacing.logicTicksPerSecond
            / ActorLocomotionPacing.infinityEngineBodyPixels
        return bodyHeightsPerSecond * OfficeInteriorScale.standingAdultBodyHeight
    }

    var isImmobile: Bool { effectiveMoveScale == nil }

    /// Duration for a path segment of the given projected length.
    func pathDuration(distance: CGFloat) -> TimeInterval {
        guard walkSpeed > 0 else { return ActorLocomotionPacing.minimumSegmentDuration }
        return TimeInterval(
            max(CGFloat(ActorLocomotionPacing.minimumSegmentDuration), distance / walkSpeed)
        )
    }
