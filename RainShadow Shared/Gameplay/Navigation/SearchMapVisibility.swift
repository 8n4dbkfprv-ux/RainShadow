...
extension SearchMap {
    ...
    func exploreMapChunk(
        ...
        let diameter = range * 2 + 1
        var marked = [UInt8](repeating: 0, count: diameter * diameter)
        let originIndex = range * diameter + range
        marked[originIndex] = 1
        ...
        for ray in 0..<perimeter {
            ...
            for distance in 0..<range {
                ...
                if !block {
                    ...
                    // GemRB `Map::ExploreMapChunk` branches in this order, and on
                    // these two different door flags. A door that blocks sight is
                    // `NO_SEE` and stops the ray outright; only a door that blocks
                    // *movement* while staying transparent can shroud. Keying the
                    // shroud off the sight flag instead — as this did — asks the
                    // opposite question, and never reaches the shroud at all.
                    if !inMap || !terrain.isSeeThrough || doorBlocksSight(at: tile) {
                        block = true
                    } else if terrain.isSightSidewall {
                        sidewall = true
                    } else if sidewall {
                        block = true
                    } else if outdoorDoorShroud && doorBlocksMovement(at: tile) {
                        fogOnly = true
                    }
                }
                if block {
                ...
                let index = (offset.dy + range) * diameter + (offset.dx + range)
                if fogOnly {
                    if marked[index] == 0 { marked[index] = 2 }
                } else {
                    marked[index] = 1
                }
            }
        ...
        var visible: Set<SearchMapCell> = []
        var exploredOnly: Set<SearchMapCell> = []
        visible.reserveCapacity(diameter * diameter / 2)
        for i in 0..<marked.count where marked[i] != 0 {
            let dx = i % diameter - range
            let dy = i / diameter - range
            let cell = SearchMapCell(column: origin.column + dx, row: origin.row + dy)
            if marked[i] == 1 {
                visible.insert(cell)
            } else {
                exploredOnly.insert(cell)
            }
        }
