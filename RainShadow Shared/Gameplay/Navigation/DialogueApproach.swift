...
/// pathfinder uses.
///
/// The solver deliberately requires a real path. `NavigationMap.route` snaps to the
/// nearest reachable cell and reports success, which is right for "walk over there" and
/// wrong here: it would happily park the detective across a wall from the person he is
/// supposedly talking to.
enum DialogueApproach {
    ...
    static func approachPoint(
        ...
        if hypot(walker.x - actor.x, walker.y - actor.y) <= standoff,
           map.path(from: walker, to: walker) != nil {
            return walker
        }
        ...
        for candidate in candidates(around: actor, facing: walker, standoff: standoff) {
            ...
            // `path` (not `route`) — a snapped "close enough" destination is exactly the
            // failure this guard exists to catch.
            guard map.path(from: walker, to: walkable) != nil else { continue }
            return walkable
        }
        return nil
    }
