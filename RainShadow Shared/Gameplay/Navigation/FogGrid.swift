...
/// GemRB levels. The runtime uploads exactly that small mask and evaluates
/// `FogRenderer::DrawFogCellVertices` / `DrawFogSmoothing` in a fragment shader,
/// so the 32-unit topology stays fixed without enlarging a coarse edge texture.
struct FogGrid: Hashable, Sendable {
    /// GemRB `FogRenderer::CELL_SIZE`. Square on the screen, and the world size
    /// of one fog cell. The uploaded state texture has one texel per cell; the
    /// geometry edge itself is evaluated at output-fragment resolution.
            ...
            for column in (x0 / pixel)...(x1 / pixel) {
                for row in (y0 / pixel)...(y1 / pixel) {
                    if column >= 0, column < columns, row >= 0, row < rows {
                        fog.insert(FogCell(column: column, row: row))
                    }
                }
            }
        }
    ...
    /// One byte per fog cell. The renderer uploads this directly; `FogEdgeMask`
    /// is the testable software reference for the fragment shader that draws it.
    var maskWidth: Int { columns }
    var maskHeight: Int { rows }

    /// One byte per cell, three levels, row 0 at the **top**. The software
    /// reference keeps that stable image order; `FogMaskRenderer` flips once when it
    /// uploads to SpriteKit's y-up data texture.
    ///
    ...
    func mask(explored: Set<FogCell>, visible: Set<FogCell>) -> [UInt8] {
        ...
        writeMask(explored: explored, visible: visible, into: &buffer)
        return buffer
    }

    func writeMask(explored: Set<FogCell>, visible: Set<FogCell>, into buffer: inout [UInt8]) {
        ...
        if buffer.count != count {
            buffer = [UInt8](repeating: Self.unexploredLevel, count: count)
        }
