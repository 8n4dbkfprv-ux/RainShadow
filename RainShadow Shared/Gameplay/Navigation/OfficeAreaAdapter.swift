...
enum OfficeAreaAdapter {
        ...
        return AreaDefinition(
            ...
            doors: [
                AreaDoor(
                    ...
                    approachPoints: Self.approachPair(
                        from: OfficeNavigationLayout.approachPoints["office.door"]
            ...
            animations: [
                AreaAnimation(
                    id: "office.windowRain",
                    point: AreaPoint(
                        OfficeInteriorScale.mapPoint(
                            OfficeNavigationLayout.AuthoredPlacement.windowRainEmitter
                        )
                    ),
                    textureName: "office_window_rain_highlight",
                    frameCount: 1,
                    alpha: 0.28,
                ...
                AreaAnimation(
                    ...
                    point: AreaPoint(
                        OfficeInteriorScale.mapPoint(
                            OfficeNavigationLayout.AuthoredPlacement.personalFan
                        )
                    ),
                    textureName: "office_fan_shadow",
                    frameCount: 1,
                    frameRate: 4,
                    alpha: 0.18,
                    wallHides: true,
    ...
    private static func wallPolygons() -> [AreaWallPolygon] {
        AreaCoverAuthoring.officeWallPolygons()
    }
    ...
    /// IE walks the nearer of two points. The second sits one body-depth
    /// camera-near of the authored standable, still on the threshold side.
    static func approachPair(from point: CGPoint) -> [AreaPoint] {
        [
            AreaPoint(point),
            AreaPoint(x: point.x, y: point.y - AreaCoverAuthoring.furnitureCoverDepth)
        ]
    }
}
