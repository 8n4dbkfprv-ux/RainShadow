...
@MainActor
final class DetectiveOfficeScene: GameAreaScene, CutsceneStage {
    ...
    // Bump abandonment and the backoff wait moved onto `Movable`, where the
    // engine keeps them: `DoStep`'s own `WithinPersonalRange` cut-off, and
    // `Backoff`'s `RAND(MAX_PATH_TRIES, MAX_PATH_TRIES * 2)`.
    ...
    override var referenceCameraScale: CGFloat { OfficeInteriorScale.cameraScaleAt100Percent }
    ...
    override func buildScene() {
        AreaLoadTrace.measure("office.lightingChannels") { loadLightingChannels() }
        AreaLoadTrace.measure("office.ambients") { buildAreaAmbients() }
        AreaLoadTrace.measure("office.animations") { buildAreaAnimations() }
        ...
        let props = AreaLoadTrace.measure("office.buildProps") { buildProps(from: area) }
        ...
        client.attachNavigation(navigation, id: Self.clientActorID)
        ...
        AreaLoadTrace.measure("office.addFogOfWar") { addFogOfWar() }

        AreaLoadTrace.measure("office.configureNavigation") { configureNavigation() }
        configureHotspots()
        AreaLoadTrace.measure("office.highlightables") { installHighlightables(OfficeHighlightOutlines.objects()) }
        installGroundCircleActors([detective, client])
    ...
    override func handlePointerUp(_ event: GamePointerEvent) {
        ...
        if let utility = portraitBar.endUtilityPress(at: portraitBar.convert(hudPoint, from: hudRoot)) {
            if utility == .search {
                toggleQuickLootBar()
                return
            }
        ...
        if let hotspot = hotspots.first(where: { $0.hitArea.contains(event.location) }) {
            ...
            if let travel = hotspot.travel {
                ...
                let fallback = hotspot.approachPoint
                let from = detective.position
                if let door {
    ...
    override func handlePointerMoved(_ event: GamePointerEvent) {
        ...
        applyWorldCursor(WorldCursorState.resolve(
            ...
            isTravel: hoveredHighlightID.flatMap { id in
                hotspots.first { $0.id == id }
            }?.travel != nil,
            hasInteractable: hoveredHighlightID != nil,
    ...
    override func update(_ currentTime: TimeInterval) {
        ...
        updateDetectiveDepth()
        updateDepth(of: client)
        applyActorCover(to: client, at: client.position, seated: false)
        ...
        // `ScreenFlags::Cutscene` — GemRB hides every ground circle but the
        // speaker's for the duration.
        detective.groundCircleState.isCutscene = cutsceneActive
        client.groundCircleState.isCutscene = cutsceneActive
        // Lila owns the only graph in the room, so while dialogue holds the world
        // she is `dialoghandler->GetTarget()` and her circle goes white.
        client.groundCircleState.isDialogueTarget = dialogueIsActive
        updateGroundCircles(at: currentTime)
        // Follow dialogue camera lifts / restores every frame.
        syncHudToCamera()
    }

    /// The painted room is this area's `area->GetSize()`, not the plate rect.
    /// An Infinity Engine area is WED-covered edge to edge, so map size and
    /// painted extent are the same thing there; the office plate is a 4096x2304
    /// canvas with the painting inset and black baked around it, and the room is
    /// the half of that split which means "the area". At play density it is
    /// smaller than the viewport in both axes, so `AreaViewport` takes its
    /// centring branch and the office camera does not pan at all — zoom is the
    /// only camera freedom this scene has.
    ...
    #if DEBUG
    /// QA: inspect a precise native seat-chain frame in the real room renderer.
    func seekVossPoseForCapture() {
        if let pose = ProcessInfo.processInfo.environment["RAINSHADOW_CAPTURE_VOSS_POSE"] {
            detective.seekPoseForCapture(pose)
        }
    }
    #endif
    ...
    /// Retire pips as their goals are walked through.
    ///
    /// `DoStep` clears a node's waypoint mark on arrival, so the pips are a pure
    ...
    ///
    /// All of the decision-making now lives inside the walker, where the engine
    /// keeps it: `DoStep` probes along its own heading — "we want to only check
    /// directly along the way and not be blocked by actors who are on the sides"
    /// — and then either pushes the blocker aside, abandons the path when it is
    /// nearly at its goal, or backs off and waits a randomised number of ticks.
    ///
    /// The scene's only remaining job is delivery. A `Movable` has no handle on
    /// its neighbours, so the `actorInTheWay->BumpAway()` the engine calls
    /// directly arrives here as an actor id to relay.
    private func processBumpRequests() {
        if let blockerID = detective.bumpRequest {
            detective.clearBumpRequest()
            if blockerID == Self.clientActorID, !client.isHidden {
                client.bumpAway()
            }
        }
    ...
    /// prop list. Its pixels and collision remain independently registered, but
    /// both resolve through the same `office.door` id.
    private func buildRegisteredDoorVisual() {
        ...
        officeDoor = sprite
        // The leaf is a live sprite in `depthWorld`; the outline layer sits below
        // it. Lift this one outline so the IE wash draws over the door it traces.
    ...
    private func addScaleReferenceStandsIfRequested() {
        guard ProcessInfo.processInfo.environment["RAINSHADOW_SCALE_RIG"] == "1" else { return }
        let library = try? IEAvatarFrameLibrary(character: "Voss")
        guard let frame = IEAvatarFrames.sequence(
            library: library,
            atlas: "VossIdle.atlas",
            stems: ["voss_standing_idle_s_00"],
            compatibilityAnchor: CGPoint(x: 0.5, y: 40 / 256)
        )?.first else { return }
        for position in OfficeNavigationLayout.scaleReferenceStands {
            let stand = IEAvatarNode(frame: frame)
    ...
    private func doorTexture(for state: OfficeDoorVisualState) -> SKTexture? {
        guard let registration = area.doors.first(where: { $0.id == "office.door" })?.visual
        else { return nil }
        let name: String
        switch state {
        case .closed: name = registration.closedTextureName
        case .mid: name = registration.midTextureName
        case .open: name = registration.openTextureName
        }
    ...
    private func presentDoorVisualState(_ state: OfficeDoorVisualState) {
        guard let officeDoor,
              ...
              let texture = doorTexture(for: state) else {
            return
        }
