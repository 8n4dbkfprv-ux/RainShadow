...
final class AreaMapOverlay: SKNode {
    ...
    private func finishInitialization() {
        ...
        AreaLoadTrace.measure("init.AreaMapOverlay") { buildInterface() }
        isHidden = true
    }
    ...
    func updateExploredFog(_ texture: SKTexture?) {
        explorationFog?.updateFogTexture(texture)
    }
...
private final class LocalMapFogNode: SKSpriteNode {
    ...
    /// The world and local map share one categorical state texture and one
    /// GemRB geometry compositor; only their display sizes differ.
    func updateFogTexture(_ texture: SKTexture?) {
        self.texture = texture
        shader = texture.map { FogMaskRenderer.makeShader(gridSize: $0.size()) }
    }
