...
final class FogOfWarNode: SKSpriteNode {
    ...
    init(
        ...
        // Clear, not black: if the first fog texture upload fails (memory), a
        // black placeholder would hide the whole plate. Unexplored cells are
        // painted opaque black by the mask texture once it lands.
        super.init(texture: nil, color: .clear, size: renderer.worldFrame.size)
        anchorPoint = .zero
        position = renderer.worldFrame.origin
        // The texture is only one categorical state texel per 32-unit cell.
        // GemRB's cardinal and diagonal raw geometry is reconstructed by this
        // shader at the output fragment, not enlarged from a baked edge mask.
        shader = renderer.makeShader()
    ...
    private func redraw() {
        if let texture = renderer.makeTexture(
            explored: exploredCells,
            visible: visibleCells,
            lastCellMask: &lastCellMask
        ) {
