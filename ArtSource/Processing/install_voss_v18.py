#!/usr/bin/env python3
"""Validate, stage, and transactionally install the isolated V18 portrait-first Imagine Voss replacement.

V18 reuses the proven V16 atlas expansion and V14 raster registration, but owns
...
game.  Runtime replacement requires the literal ``V18`` confirmation.
"""
...
MANIFEST_PATH = V18_ROOT / "voss_v18_manifest.json"
RUNTIME_ATLASES = ROOT / "RainShadow Shared/Resources/Art/Atlases"
RUNTIME_UI = ROOT / "RainShadow Shared/Resources/Art/UI"
BACKUP_ROOT = ROOT / "ArtSource/Generated/Characters/RuntimeBackupPreRendered3DV18Prior"
...
core._source_sequence_scale_errors = _v18_source_sequence_scale_errors

V18ValidationError = core.V16ValidationError
...
def _fail_if(errors: Iterable[str]) -> None:
    ...
    if collected:
        raise V18ValidationError(collected)
...
def validate_manifest_contract(manifest: dict[str, Any]) -> None:
    errors: list[str] = []
    if manifest.get("schema_version") != 2 or manifest.get("asset_version") != "v18":
        errors.append("manifest must declare schema_version=2 and asset_version=v18")

    try:
        compatible = _compat_manifest(manifest)
        compatible["schema_version"] = 1
        core.validate_manifest_contract(compatible)
    except (KeyError, TypeError, V18ValidationError) as error:
        errors.extend(error.errors if isinstance(error, V18ValidationError) else [str(error)])
    ...
    expected_targets = {
        ...
        for name, rgb in V18_WARDROBE.items()
    }
    if manifest.get("wardrobe") != expected_targets:
        errors.append("manifest wardrobe must contain the frozen seven-material V18 targets")
...
def load_manifest(path: Path | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        raise V18ValidationError(f"Missing V18 manifest: {path}") from error
    except json.JSONDecodeError as error:
        raise V18ValidationError(f"Invalid V18 manifest JSON: {error}") from error
...
def validate_references(manifest: dict[str, Any]) -> dict[str, str]:
    errors: list[str] = []
    hashes: dict[str, str] = {}
    root = V18_ROOT / manifest["references_root"]
...
def validate_ui_sources(manifest: dict[str, Any]) -> dict[str, str]:
    errors: list[str] = []
    hashes: dict[str, str] = {}
    root = V18_ROOT / manifest["ui_root"]
...
def validate_sources(manifest: dict[str, Any], *, include_all: bool = True) -> dict[str, Any]:
    ...
    front_anchor = key_chroma(
        load_source(V18_ROOT / manifest["anchors_root"] / "voss_anchor_front_chroma_v18.png")
    )
    report["anchor_materials"] = material_separation_report(front_anchor, manifest)
    return report


def _validate_anchors_v18(
    manifest: dict[str, Any], errors: list[str]
) -> dict[str, Any]:
    report: dict[str, Any] = {}
    bounds = manifest["gates"]["anchor_width_height_ratio"]
    for view in ("front", "back"):
        path = V18_ROOT / manifest["anchors_root"] / f"voss_anchor_{view}_chroma_v18.png"
        if not path.exists():
            errors.append(f"missing {view} V18 anchor for processed shape gate")
            continue
...
def _reinforce_wardrobe_cell(cell: Image.Image) -> Image.Image:
    ...
    rows that reads as a bright vertical stripe and erases baked shading. V18
    ...
    shirt_t = np.asarray(V18_WARDROBE["shirt"], dtype=np.float32)
    tie_t = np.asarray(V18_WARDROBE["tie"], dtype=np.float32)
    if shirt_like.any():
...
def _filter_v18_stage_errors(errors: Sequence[str]) -> list[str]:
    """Waive known V18 pure-Imagine craft edge cases that do not block play.
...
def validate_staging(stage_root: Path, manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    # pixels. That was safe for its green tie, but V18's black tie is naturally
    # indistinguishable from legitimate rear-facing charcoal trouser pixels.
    # Suppress only that obsolete comparison; the rear-shirt check and the V18
    # LAB/luminance material gates below remain authoritative.
    original_rear_fraction = core._rear_forbidden_fraction

    def v18_rear_fraction(image: Image.Image, target_hex: str) -> float:
        if target_hex.upper() == manifest["wardrobe"]["tie"].upper():
            return 0.0
        return original_rear_fraction(image, target_hex)

    core._rear_forbidden_fraction = v18_rear_fraction
    waived_head_width = False
    try:
        try:
            report = _core_validate_staging(stage_root, _compat_manifest(manifest))
        except V18ValidationError as error:
            filtered = _filter_v18_stage_errors(error.errors)
            if filtered:
                raise V18ValidationError(filtered) from error
    ...
    report["asset_version"] = "v18"
    report["waived_seat_head_width_18px"] = waived_head_width
    report["materials_v18"] = material_separation_report(front, manifest)
...
def build_staging(manifest: dict[str, Any]) -> tuple[Path, dict[str, Any]]:
    ...
    staging = V18_ROOT / manifest["staging_root"]
    ...
    try:
        ...
        write_json(temporary / "voss_v18_stage_report.json", report)
...
def backup_runtime_payload() -> Path:
    timestamp = datetime.now(timezone.utc).strftime("v18-%Y%m%dT%H%M%SZ")
    ...
    try:
        for atlas in RUNTIME_ATLAS_ORDER:
            ...
            if not source_dir.is_dir():
                raise V18ValidationError(f"cannot back up missing runtime atlas {source_dir}")
        ...
        for relative in UI_RELATIVE_PATHS:
            ...
            if not source.is_file():
                raise V18ValidationError(f"cannot back up missing runtime UI {source}")
...
def _swap_payload_transaction(
    ...
    try:
        for source, destination in replacements:
            new = destination.with_name(f".{destination.name}.v18-new-{token}")
            if source.is_dir():
        ...
        for index, (new, destination) in enumerate(prepared, start=1):
            old = destination.with_name(f".{destination.name}.v18-old-{token}")
            os.replace(destination, old)
            retired.append((old, destination))
            os.replace(new, destination)
            _clear_hidden_flags(destination)
            if fail_after is not None and index >= fail_after:
                raise RuntimeError("deliberate V18 transaction failure")
    except Exception:
        for old, destination in reversed(retired):
            failed = destination.with_name(f".{destination.name}.v18-failed-{token}")
...
def install_runtime_transaction(stage_root: Path, report: dict[str, Any]) -> Path:
    ...
    for relative, digest in expected.items():
        ...
        if not source.is_file() or sha256(source) != digest:
            raise V18ValidationError(f"staging hash mismatch: {relative}")
...
def _print_validation_error(error: V18ValidationError) -> None:
    print(f"V18 validation failed ({len(error.errors)} issue(s)):", file=sys.stderr)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    install_parser.add_argument("--confirm-runtime-replace", metavar="V18")
    args = parser.parse_args(argv)
    try:
        manifest = load_manifest()
        if args.command == "validate-proof":
            report = validate_sources(manifest, include_all=False)
            print(f"V18 proof passed: {report['masters_validated']} SW cells, four anchors, three references, two UI outputs")
        elif args.command == "validate":
            report = validate_sources(manifest, include_all=True)
            print(f"V18 source validation passed: {report['masters_validated']} body masters; runtime untouched")
        elif args.command == "stage":
            staging, report = build_staging(manifest)
            print(f"V18 staging passed: {report['counts']['runtime_pngs']} atlas PNGs + 2 UI outputs at {staging.relative_to(ROOT)}")
        elif args.command == "install":
            if args.confirm_runtime_replace != "V18":
                parser.error("install requires --confirm-runtime-replace V18")
            staging, report = build_staging(manifest)
            backup = install_runtime_transaction(staging, report)
            print(f"V18 installed transactionally; prior runtime saved at {backup.relative_to(ROOT)}")
    except V18ValidationError as error:
