#!/usr/bin/env python3
"""Preflight and atomically install the BG:EE-locked V09 cramped office."""
...
PLATE = SOURCE / "office_cramped_plate_v09.png"
MASK = SOURCE / "office_cramped_architecture_mask_v09.png"
METRICS = SOURCE / "office_cramped_metrics_v09.json"
GEOMETRY = SOURCE / "office_v09_geometry.json"
DOOR_FAMILY = SOURCE_PROPS / "office_door_family_v09.json"

DOOR_INSTALLS = {
    "office_door_leaf_closed_v09.png": "office_door_leaf.png",
    "office_door_leaf_closed_hover_v09.png": "office_door_leaf_hover.png",
    "office_door_leaf_mid_v09.png": "office_door_leaf_mid.png",
    "office_door_leaf_open_v09.png": "office_door_leaf_open.png",
    "office_door_leaf_open_hover_v09.png": "office_door_leaf_open_hover.png",
}
...
def open_state_black_fraction() -> float:
    ...
    with Image.open(SOURCE_PROPS / "office_door_leaf_open_v09.png") as source:
...
def preflight() -> dict:
    ...
    if missing:
        raise RuntimeError("missing V09 inputs: " + ", ".join(missing))
    ...
    # Hover states are tint-only. Their alpha contract is exactly identical.
    for state in ("closed", "open"):
        base = np.asarray(Image.open(SOURCE_PROPS / f"office_door_leaf_{state}_v09.png"))
        hover = np.asarray(Image.open(SOURCE_PROPS / f"office_door_leaf_{state}_hover_v09.png"))
    ...
    if metrics.get("geometryManifest") != GEOMETRY.name:
        raise RuntimeError("candidate does not name the V09 geometry manifest")
...
def _atomic_copy(source: Path, target: Path) -> None:
    ...
    temporary = target.with_name(target.name + ".v09-installing")
...
def main() -> None:
    ...
    if not args.install:
        print("V09 preflight passed; no runtime files changed (use --install)")
        return
    ...
    plate_targets = [
        ...
        GENERATED_OFFICE / "office_suite_plate_bgee_v09_installed.png",
    ]
    ...
    out = SOURCE / "office_cramped_install_v09.json"
