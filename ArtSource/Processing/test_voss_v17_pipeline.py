#!/usr/bin/env python3
"""Asset-independent tests for the V17 contract and transactional plumbing."""
...
class VossV17PipelineTests(unittest.TestCase):
    ...
    def test_manifest_expands_to_exact_runtime_contract(self) -> None:
        ...
        runtime = v17.expected_runtime_names()
    ...
    def test_reference_hashes_are_immutable(self) -> None:
        hashes = v17.validate_references(v17.load_manifest())
        self.assertEqual(len(hashes), 3)
        self.assertTrue(all(len(value) == 64 for value in hashes.values()))

    def test_material_targets_are_lab_and_luminance_separated(self) -> None:
        manifest = v17.load_manifest()
        report = v17.material_separation_report(v17.key_chroma(synthetic_master()), manifest)
    ...
    def test_chroma_key_keeps_black_tie_and_removes_green(self) -> None:
        ...
        ImageDraw.Draw(image).rectangle((13, 4, 18, 27), fill=v17.V17_WARDROBE["tie"])
        keyed = np.asarray(v17.key_chroma(image))
    ...
    def test_v14_registration_is_200px_hard_alpha(self) -> None:
        cell = v17.process_figure(synthetic_master())
        metrics = v17.frame_metrics(cell)
    ...
    def test_compatibility_layer_union_is_exact(self) -> None:
        ...
        union = v17.visible_mask(upper) | v17.visible_mask(lower)
        self.assertTrue(np.array_equal(union, v17.visible_mask(cell)))

    def test_ui_dimensions_and_modes(self) -> None:
        hashes = v17.validate_ui_sources(v17.load_manifest())
    ...
    def test_transaction_rolls_back_deliberate_failure(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with self.assertRaisesRegex(RuntimeError, "deliberate V17"):
                v17._swap_payload_transaction(
    ...
    def test_manifest_rejects_old_mustard_identity(self) -> None:
        ...
        with self.assertRaises(v17.V17ValidationError):
            v17.validate_manifest_contract(manifest)
