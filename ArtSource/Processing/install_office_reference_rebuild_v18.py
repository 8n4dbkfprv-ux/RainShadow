#!/usr/bin/env python3
"""Read-only preflight and atomic installer for the V18 radiator office."""
...
BASE_PLATE = STAGE / "office_reference_rebuild_plate_v18.png"
BAKED_PLATE = BAKE / "office_suite_plate_baked_v18.png"
BAKE_MANIFEST = BAKE / "office_plate_bake_manifest_v18.json"
GLASS = STAGE / "office_window_glass_mask_v18.png"
HOVER = STAGE / "office_window_near_hover_overlay_v18.png"
METRICS = STAGE / "office_reference_rebuild_metrics_v18.json"
...
ALLOWLIST: tuple[tuple[Path, Path], ...] = (
    ...
    (BAKED_PLATE, GENERATED_OFFICE / "office_suite_plate_bgee_v18_installed.png"),
    (GLASS, RUNTIME_PROPS / "office_window_glass_mask.png"),
    (HOVER, RUNTIME_PROPS / "office_window_hover_overlay.png"),
    (METRICS, GENERATED_OFFICE / "office_reference_rebuild_metrics_v18.json"),
)
...
def preflight() -> dict[str, object]:
    ...
    if missing:
        raise RuntimeError("missing V18 allowlist inputs: " + ", ".join(missing))
    for script in ("qa_office_reference_rebuild_v18.py", "office_layout_plan.py"):
        subprocess.run(
    ...
    if manifest.get("version") != 18:
        raise RuntimeError("V18 installer requires the V18 bake manifest")
    if manifest.get("bakedPlateSHA256") != installer.sha256(BAKED_PLATE):
        raise RuntimeError("V18 baked plate no longer matches its manifest")
    return {
        "version": "BGEEReferenceV18",
...
def main() -> None:
    ...
    if not args.install:
        print(f"V18 preflight passed for {len(ALLOWLIST)} allowlisted targets")
        print("no runtime files changed (use --install)")
        return

    # Reuse the audited V17 atomic-copy implementation with the V18 allowlist.
    ...
    provenance_path = STAGE / "office_reference_rebuild_install_v18.json"
    provenance_path.write_text(json.dumps(provenance, indent=2, sort_keys=True) + "\n")
    print(f"installed {len(ALLOWLIST)} exact V18 allowlist targets")
