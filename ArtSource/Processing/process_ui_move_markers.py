...
def fit_canvas(im: Image.Image, size: tuple[int, int]) -> Image.Image:
    ...
    scale = min(cw / im.width, ch / im.height) * 0.92
...
def derive_frames(base: Image.Image, prefix: str) -> None:
    ...
    # Scale goes 1.18 → 0.78 and alpha 0.55 → 1.0 → 0.35 across the loop.
    for index in range(FRAME_COUNT):
        t = index / max(1, FRAME_COUNT - 1)
        # Ease: expand then settle (first half grow-in, second half contract).
        if t < 0.35:
            local = t / 0.35
            scale = 1.18 - 0.22 * local
            alpha = 0.55 + 0.45 * local
        else:
            local = (t - 0.35) / 0.65
            scale = 0.96 - 0.18 * local
            alpha = 1.0 - 0.65 * local
...
def process_marker(master_name: str, prefix: str, size: tuple[int, int]) -> Image.Image:
    ...
    fitted = fit_canvas(trimmed, size)
