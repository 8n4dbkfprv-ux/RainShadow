#!/usr/bin/env python3
"""Read-only preflight and atomic installer for the furnished V13 office."""
...
MAP_SOURCE = ROOT / "ArtSource/Generated/UI/Map/map_detective_office_v13.png"
...
BAKED_PLATE = BAKE / "office_suite_plate_baked_v13.png"
BAKE_MANIFEST = BAKE / "office_plate_bake_manifest_v13.json"
GLASS = STAGE / "office_window_glass_mask_v13.png"
HOVER = STAGE / "office_window_near_hover_overlay_v13.png"
...
ALLOWLIST: tuple[tuple[Path, Path], ...] = (
    ...
    (BAKED_PLATE, GENERATED_OFFICE / "office_suite_plate_bgee_v13_installed.png"),
    (GLASS, RUNTIME_PROPS / "office_window_glass_mask.png"),
    (HOVER, RUNTIME_PROPS / "office_window_hover_overlay.png"),
    (METRICS, GENERATED_OFFICE / "office_reference_rebuild_metrics_v13.json"),
...
def preflight() -> dict[str, object]:
    ...
    if missing:
        raise RuntimeError("missing V13 allowlist inputs: " + ", ".join(missing))

    for script in (
        "qa_office_reference_rebuild_v13.py",
        "qa_office_layout_v13.py",
    ...
    if bake_manifest.get("version") != 13:
        raise RuntimeError("V13 installer requires the V13 bake manifest")
    if bake_manifest.get("bakedPlateSHA256") != sha256(BAKED_PLATE):
        raise RuntimeError("V13 baked plate no longer matches its manifest")

    targets = [target.resolve() for _, target in ALLOWLIST]
    if len(targets) != len(set(targets)):
        raise RuntimeError("V13 installer allowlist contains duplicate targets")
    for target in targets:
        target.relative_to(ROOT.resolve())

    return {
        "version": "BGEEReferenceV13",
...
def _stage_atomic_copies() -> list[tuple[Path, Path]]:
    ...
    try:
        for source, target in ALLOWLIST:
            ...
            temporary = target.with_name(target.name + ".v13-installing")
...
def _commit_atomic_copies(staged: list[tuple[Path, Path]]) -> None:
    ...
    try:
        for temporary, target in staged:
            backup = target.with_name(target.name + ".v13-rollback")
...
def main() -> None:
    ...
    if not args.install:
        print(f"V13 preflight passed for {len(ALLOWLIST)} allowlisted targets")
    ...
    provenance_path = STAGE / "office_reference_rebuild_install_v13.json"
    provenance_path.write_text(
        json.dumps(provenance, indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
    print(f"installed {len(ALLOWLIST)} exact V13 allowlist targets")
