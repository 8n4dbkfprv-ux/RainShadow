#!/usr/bin/env python3
"""Generate V17 gate reports, contact sheets, previews, and 0.25x walk GIFs.

Every output lands under ``PreRendered3DV17/QA``.  Runtime atlases are never read
or written: this script reviews only the gated V17 staging tree and immutable
...
ROOT = v17.ROOT
V17_ROOT = v17.V17_ROOT
PAPERDOLL = V17_ROOT / "UI/voss_paperdoll_front_rgba_v01.png"
PORTRAIT = V17_ROOT / "UI/dialogue_portrait_harlan_voss_v01.png"
...
def make_identity_shape_sheet(stage: Path, manifest: dict[str, Any], qa: Path) -> Path:
    anchor_root = V17_ROOT / manifest["anchors_root"]
    front_anchor = v17.key_chroma(load_rgba(anchor_root / "voss_anchor_front_chroma_v17.png"))
    profile_anchor = v17.key_chroma(load_rgba(anchor_root / "voss_anchor_profile_w_chroma_v17.png"))
    back_anchor = v17.key_chroma(load_rgba(anchor_root / "voss_anchor_back_chroma_v17.png"))
    ...
    sheet = grid(tiles, 6)
    path = qa / "qa_v17_front_profile_back_identity_shape.png"
...
def make_facing_sheets(stage: Path, qa: Path) -> list[Path]:
    ...
    paths = [
        qa / "qa_v17_16_facings_unlabelled.png",
        qa / "qa_v17_16_facings_labelled.png",
    ]
    ...
    key_path = qa / "qa_v17_mirrored_se_authored_rear_nw.png"
...
def make_sw_proof(stage: Path, qa: Path) -> Path:
    ...
    path = qa / "qa_v17_sw_walk_proof.png"
...
def make_inventory_preview(stage: Path, qa: Path) -> Path:
    if not PAPERDOLL.is_file():
        raise v17.V17ValidationError(f"missing V17 paperdoll {PAPERDOLL}")
    ...
    path = qa / "qa_v17_inventory_220x315_vs_actor_180x180.png"
...
def make_dialogue_hud_preview(qa: Path) -> Path:
    if not PORTRAIT.is_file():
        raise v17.V17ValidationError(f"missing V17 portrait {PORTRAIT}")
    ...
    path = qa / "qa_v17_dialogue_hud_preview.png"
...
def make_seat_transition_strips(stage: Path, qa: Path) -> list[Path]:
    paths: list[Path] = []
    for direction in v17.SEAT_DIRECTIONS:
        cells = [
            stage_cell(
                stage,
        ...
        path = qa / f"qa_v17_stand_up_{direction}_strip.png"
...
def make_world_previews(stage: Path, qa: Path) -> list[Path]:
    ...
    paths = [qa / "qa_v17_office_actor_180x180.png", qa / "qa_v17_city_actor_180x180.png"]
    ...
    seat_path = qa / "qa_v17_seated_one_world_chair.png"
...
def make_walk_gifs(stage: Path, qa: Path) -> list[Path]:
    output: list[Path] = []
    for direction in v17.WESTERN_DIRECTIONS:
        ...
        path = qa / f"qa_v17_walk_{direction}_quarter_speed.gif"
...
def generate(stage: Path, manifest: dict[str, Any], *, allow_failing_gates: bool) -> dict[str, Any]:
    qa = V17_ROOT / manifest["qa_root"]
    qa.mkdir(parents=True, exist_ok=True)
    gate_failures: list[str] = []
    try:
        gate_report = v17.validate_staging(stage, manifest)
    except v17.V17ValidationError as error:
    ...
    v17.write_json(qa / "qa_v17_report.json", report)
    return report
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    stage = args.staging or (V17_ROOT / manifest["staging_root"])
    try:
        report = generate(stage, manifest, allow_failing_gates=args.allow_failing_gates)
    except v17.V17ValidationError as error:
        v17._print_validation_error(error)
        return 1
    print(
        f"V17 QA {report['status']}: {len(report['outputs'])} review files under "
        f"{(V17_ROOT / manifest['qa_root']).relative_to(ROOT)}"
    )
