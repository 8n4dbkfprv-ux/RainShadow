#!/usr/bin/env python3
"""Generate V16 gate reports, contact sheets, previews, and 0.25x walk GIFs.

Every output lands under ``PreRendered3DV16/QA``.  Runtime atlases are never read
or written: this script reviews only the gated V16 staging tree and immutable
...
import install_voss_v16 as v16  # noqa: E402


ROOT = v16.ROOT
V16_ROOT = v16.V16_ROOT
PAPERDOLL = ROOT / "RainShadow Shared/Resources/Art/UI/Inventory/voss_paperdoll_front_rgba_v01.png"
...
def make_identity_shape_sheet(stage: Path, manifest: dict[str, Any], qa: Path) -> Path:
    anchor_root = V16_ROOT / manifest["anchors_root"]
    front_anchor = v16.key_chroma(load_rgba(anchor_root / "voss_anchor_front_chroma_v16.png"))
    back_anchor = v16.key_chroma(load_rgba(anchor_root / "voss_anchor_back_chroma_v16.png"))
    ...
    tiles = [
        tile(front_anchor, "front anchor / identity + shape", smooth=True),
        tile(front_cell, "processed front / V14 crunch"),
        tile(back_anchor, "back anchor / shape only", smooth=True),
        tile(back_cell, "processed rear / no shirt or tie"),
    ]
    sheet = grid(tiles, 4)
    draw = ImageDraw.Draw(sheet)
    for index, image in enumerate((front_cell, back_cell)):
        metrics = v16.frame_metrics(image)
        ratio = metrics.width / metrics.height
        x = (index * 2 + 1) * (tiles[0].width + 6) + 8
        label(draw, (x, sheet.height - 18), f"ratio {ratio:.3f}")
    path = qa / "qa_v16_front_back_identity_shape.png"
...
def make_facing_sheets(stage: Path, qa: Path) -> list[Path]:
    ...
    paths = [
        qa / "qa_v16_16_facings_unlabelled.png",
        qa / "qa_v16_16_facings_labelled.png",
    ]
    ...
    key_path = qa / "qa_v16_mirrored_se_authored_rear_nw.png"
...
def make_sw_proof(stage: Path, qa: Path) -> Path:
    ...
    path = qa / "qa_v16_sw_walk_proof.png"
...
def make_inventory_preview(stage: Path, qa: Path) -> Path:
    if not PAPERDOLL.is_file():
        raise v16.V16ValidationError(f"missing runtime paperdoll {PAPERDOLL}")
    ...
    path = qa / "qa_v16_inventory_220x315_vs_actor_180x180.png"
...
def make_world_previews(stage: Path, qa: Path) -> list[Path]:
    ...
    paths = [qa / "qa_v16_office_actor_180x180.png", qa / "qa_v16_city_actor_180x180.png"]
    ...
    seat_path = qa / "qa_v16_seated_one_world_chair.png"
...
def make_walk_gifs(stage: Path, qa: Path) -> list[Path]:
    output: list[Path] = []
    for direction in v16.WESTERN_DIRECTIONS:
        ...
        path = qa / f"qa_v16_walk_{direction}_quarter_speed.gif"
...
def generate(stage: Path, manifest: dict[str, Any], *, allow_failing_gates: bool) -> dict[str, Any]:
    qa = V16_ROOT / manifest["qa_root"]
    qa.mkdir(parents=True, exist_ok=True)
    gate_failures: list[str] = []
    try:
        gate_report = v16.validate_staging(stage, manifest)
    except v16.V16ValidationError as error:
    ...
    v16.write_json(qa / "qa_v16_report.json", report)
    return report
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    stage = args.staging or (V16_ROOT / manifest["staging_root"])
    try:
        report = generate(stage, manifest, allow_failing_gates=args.allow_failing_gates)
    except v16.V16ValidationError as error:
        v16._print_validation_error(error)
        return 1
    print(
        f"V16 QA {report['status']}: {len(report['outputs'])} review files under "
        f"{(V16_ROOT / manifest['qa_root']).relative_to(ROOT)}"
    )
