#!/usr/bin/env python3
"""Generate V18 gate reports, contact sheets, previews, and 0.25x walk GIFs.

Every output lands under ``PreRendered3DV18/QA``.  Runtime atlases are never read
or written: this script reviews only the gated V18 staging tree and immutable
...
import install_voss_v18 as v18  # noqa: E402


ROOT = v18.ROOT
V18_ROOT = v18.V18_ROOT
PAPERDOLL = V18_ROOT / "UI/voss_paperdoll_front_rgba_v01.png"
PORTRAIT = V18_ROOT / "UI/dialogue_portrait_harlan_voss_v01.png"
...
def make_identity_shape_sheet(stage: Path, manifest: dict[str, Any], qa: Path) -> Path:
    anchor_root = V18_ROOT / manifest["anchors_root"]
    front_anchor = v18.key_chroma(load_rgba(anchor_root / "voss_anchor_front_chroma_v18.png"))
    profile_anchor = v18.key_chroma(load_rgba(anchor_root / "voss_anchor_profile_w_chroma_v18.png"))
    back_anchor = v18.key_chroma(load_rgba(anchor_root / "voss_anchor_back_chroma_v18.png"))
    ...
    sheet = grid(tiles, 6)
    path = qa / "qa_v18_front_profile_back_identity_shape.png"
...
def make_facing_sheets(stage: Path, qa: Path) -> list[Path]:
    ...
    paths = [
        qa / "qa_v18_16_facings_unlabelled.png",
        qa / "qa_v18_16_facings_labelled.png",
    ]
    ...
    key_path = qa / "qa_v18_mirrored_se_authored_rear_nw.png"
...
def make_sw_proof(stage: Path, qa: Path) -> Path:
    ...
    path = qa / "qa_v18_sw_walk_proof.png"
...
def make_inventory_preview(stage: Path, qa: Path) -> Path:
    if not PAPERDOLL.is_file():
        raise v18.V18ValidationError(f"missing V18 paperdoll {PAPERDOLL}")
    ...
    path = qa / "qa_v18_inventory_220x315_vs_actor_180x180.png"
...
def make_dialogue_hud_preview(qa: Path) -> Path:
    if not PORTRAIT.is_file():
        raise v18.V18ValidationError(f"missing V18 portrait {PORTRAIT}")
    ...
    path = qa / "qa_v18_dialogue_hud_preview.png"
...
def make_seat_transition_strips(stage: Path, qa: Path) -> list[Path]:
    paths: list[Path] = []
    for direction in v18.SEAT_DIRECTIONS:
        cells = [
            stage_cell(
                stage,
        ...
        path = qa / f"qa_v18_stand_up_{direction}_strip.png"
...
def make_world_previews(stage: Path, qa: Path) -> list[Path]:
    ...
    paths = [qa / "qa_v18_office_actor_180x180.png", qa / "qa_v18_city_actor_180x180.png"]
    ...
    seat_path = qa / "qa_v18_seated_one_world_chair.png"
...
def make_walk_gifs(stage: Path, qa: Path) -> list[Path]:
    output: list[Path] = []
    for direction in v18.WESTERN_DIRECTIONS:
        ...
        path = qa / f"qa_v18_walk_{direction}_quarter_speed.gif"
...
def generate(stage: Path, manifest: dict[str, Any], *, allow_failing_gates: bool) -> dict[str, Any]:
    qa = V18_ROOT / manifest["qa_root"]
    qa.mkdir(parents=True, exist_ok=True)
    gate_failures: list[str] = []
    try:
        gate_report = v18.validate_staging(stage, manifest)
    except v18.V18ValidationError as error:
    ...
    v18.write_json(qa / "qa_v18_report.json", report)
    return report
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    stage = args.staging or (V18_ROOT / manifest["staging_root"])
    try:
        report = generate(stage, manifest, allow_failing_gates=args.allow_failing_gates)
    except v18.V18ValidationError as error:
        v18._print_validation_error(error)
        return 1
    print(
        f"V18 QA {report['status']}: {len(report['outputs'])} review files under "
        f"{(V18_ROOT / manifest['qa_root']).relative_to(ROOT)}"
    )
