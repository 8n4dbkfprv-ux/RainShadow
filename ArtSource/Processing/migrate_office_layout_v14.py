#!/usr/bin/env python3
"""Install V14 prop positions idempotently from frozen source metadata."""
...
# Static scenery is baked into the V14 plate.  The area record retains only
...
def _atomic_json_write(path: Path, document: dict) -> None:
    temporary = path.with_name(path.name + ".v14-installing")
...
def main() -> None:
    ...
    if missing_live:
        raise RuntimeError(f"live desk props missing from V14 source: {sorted(missing_live)}")
