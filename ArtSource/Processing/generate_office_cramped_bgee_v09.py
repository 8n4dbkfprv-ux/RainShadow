"""Build the reference-locked V09 cramped detective-office architecture.
...
OUT = SOURCE / "office_cramped_plate_v09.png"
GRAYBOX = SOURCE / "office_cramped_graybox_v09.png"
ARCHITECTURE_MASK = SOURCE / "office_cramped_architecture_mask_v09.png"
METRICS = SOURCE / "office_cramped_metrics_v09.json"
GEOMETRY = SOURCE / "office_v09_geometry.json"
FLOOR_SOURCE = SOURCE / "floor_material_source_v09.png"
WALL_SOURCE = SOURCE / "wall_material_source_v09.png"
...
def _draw_cutaways_and_threshold(image: Image.Image) -> None:
    ...
    # The V09 reference lock requires pure black beyond the cutaway. Hallway
    # light, UI, and neighbouring rooms are deliberately not part of the plate.
...
def _architecture_mask(graybox: Image.Image) -> Image.Image:
    """Freeze the exact room silhouette as a binary, immutable V09 contract."""
...
def main() -> None:
    if not FLOOR_SOURCE.exists() or not WALL_SOURCE.exists():
        raise SystemExit("missing ImageGen material sources for V09")
    ...
    # The compositor may paint only inside the frozen mask. This also makes
    # the exterior byte-for-byte black, rather than merely visually dark.
