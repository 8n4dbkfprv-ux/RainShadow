...
- Choice fields: `text` **or** `textKey`, `destinationID`, optional `destinationGraphID` (IE **EXTERN** — choices only, as in DLG), `tone`, `intention` (GDD §7.5: `open` / `press` / `feign` / `trade` / `observe` / `leave` — **author method**, not painted on the reply row), `conditions`, `gateDisclosure` (evidence/knowledge disclosure only; leftover `"Press"` is stripped), `onSelect`
