...
| Office suite plate | V20 BG:EE affine re-fit of the approved V03 noir concept | 4096×2304 @ env 0.395 | `BGEEReferenceV20/office_reference_rebuild_plate_v20.png` is the input to `bake_office_plate.py`; all scenery is embedded in the plate and it measures +36.75°/−36.56° (0.31° worst delta on the ±0.75 lock). The registered door leaf states remain live overlays. V12/V17 remain geometry/rollback provenance. |
| Full-canvas overlays | 2× listed runtime | Listed runtime | Preserve exact pixel registration with base. |
| Actor frame | Generator master | Native indexed crop plus 512×512 compatibility atlas | Reduce to a 64px native body, widen torso/coat rows by 15% while keeping the head unchanged and ramping back to 1.0x through the lower body, harden alpha to a 1-bit silhouette, limit it to seven 12-shade material ranges without dithering, and register it against the fixed 200px body footprint. The indexed runtime resolves the native crop directly and SpriteKit performs the sole linear enlargement. |
...
|---|---|---:|---|---|
| P0 | `<district plate>.pages.json` | 5×5 pages | — | Active literal BG:EE 80×60 area manifest. World 5120×3840; each page covers 1024×768 world units. |
| P0 | `<district plate>_page_c##_r##` | 2048×1536 | Opaque | Active camera-paged area painting at 2.00 installed px/world-unit. 25 pages per plate; Sable Row has day and night sets. |
| P1 | `city_<district>_block_v02` | 8192×6144 | Opaque | Registered 4096×3072 core/legacy fallback. Runtime prefers the matching page manifest. |
| P1 | `city_<district>_ground_v02` | 8192×6144 | Opaque | Legacy streets underlay and generation authority for the preserved core. |
...
| Priority | Runtime ID | Pixels | Alpha | Description |
|---|---|---:|---|---|
| P0 | `office_shell_base` | 4096×2304 | Opaque | Installed V20 re-registration of the V19 shell onto the BG:EE ±0.75 lock (0.20° worst delta): empty reference room, two small high windows, repaired hearth wall/floor, baked radiators. Pure black outside; no furniture or door pixels. |
| P0 | `office_suite_plate` | 4096×2304 | Opaque | Installed V20 re-registration of the approved V03 noir concept onto the BG:EE ±0.75 lock (0.31° worst delta); desk, chair, rug and wall fixtures are baked in. Entrance-leaf states remain registered live overlays. |
...
| P0 | `office_window_hover_overlay` | 4096×2304 | **Deprecated** | Former V14 full-plate hover overlay. Runtime highlights now use Infinity Engine outline polygons (`OfficeHighlightOutlinePolygons`, hand-traced over the V20 plate); the PNG remains on disk as inert provenance. |
...
| P0 | closed/mid/open hover states | 512×320 | **Deprecated** | Former colour-only hover derivations of each leaf. Runtime door highlight is the `office.door` outline polygon; PNGs remain on disk unused. |

The city portal leaves (`city_door_*.png` / `_mid` / `_open`, 256×384 RGBA in
`Art/Props/CityDistrict/V2/`) carry a second role beyond display: their alpha is
the trace source for the ARE outline rings in `CityHighlightOutlinePolygons`,
seated through the same `AreaDoorVisual` registration the runtime draws them
with. Regenerate with `ArtSource/Processing/generate_city_highlight_outline_polygons.py`
after any change to a leaf's silhouette or its registration, and check the
`--qa` composites in `ArtSource/Generated/CityDistrict/V2/HighlightOutlines/`.
...
At 100% play zoom, the **64-row native standing body displays at 64 logical screen points**, independent of window height. `DefaultPlayZoom` derives camera scale from `renderedStandingDetectiveBodyHeight` (70.3125 world units) divided by 64; the old 9%-of-window target is retired. Player zoom affects the whole scene and resizing reveals more world. Registered world geometry, pivots, palette and masks are unchanged. Voss's indexed plane resolves directly to native RGBA and uses SpriteKit linear sampling, without Super xBR. See `NativeSpriteCameraCalibration.md` for the engine evidence, Retina handling and subpixel crop-rounding limits; current V13 asset authority remains `ImportedVossReplacementRuntimeV13.md`.
...
The two approved departure pose strips take their emerald garment chroma from
the installed V11 arrival strip after final palette imposition; this preserves
the facing masters while preventing an in-scene costume change.
...
- Apparent standing actor height at 100% play zoom: 64 logical points from the 64-row native body, independent of window height. World proportions and registered animation pivots must not change.
...
- Floor ripples use the scene's ground ellipse (16:12 under BG:EE), not circular top-down rings.
...
| P0 | `ui_move_marker_00...07` | 8 | 128×64 | **Shipped.** Muted isometric teal ground marker converging loop on valid move orders. Becomes 128×96 with a 16:12 ellipse at BG:EE adoption. |
...
| — | ~~`ui_selection_ring_party`~~, ~~`ui_selection_ring_npc`~~ | — | — | **Retired 2026-08-28.** The ground circle is no longer art. `GroundCircleNode` strokes the ellipse `Selectable::DrawCircle` draws, in the colour `Actor::SetCircleSize` resolves each frame, so a baked-in colour could not express it. `process_ui_selection_rings.py` went with them. |
...
| P0 | `inventory_outer_frame_v16` | 1 | 1960×1080 | **Active.** Literal Mac OS 9 Platinum window grammar in RainShadow noir: 39px title strip (~3.6% of 1080), six hairlines, snug rectangular unstriped title gap (slightly lighter field, no pill/plaque), live nested-square close, disabled Zoom/WindowShade, slim single-bevel rails, sunken content well. Live INVENTORY label composites into the gap. |
...
| **P0** | **`Resources/Items/harborpoint.items.json`** | Authored item catalog (`ITM` header analogue): paired unidentified/identified name and description, category, weight in ounces, price in pence, stack amount, lore-to-identify, inventory icon, and behaviour flags. Loaded by `ItemCatalogLoader`; schema versioned like the dialogue packages. See [Inventory System Roadmap](InventorySystemRoadmap.md). |
...
| **P0** | **`dlg.inspect.office.*` keys in `strings.en.json`** | InfoPoint examine strings for office hotspots. Presented by `presentDisplayString`, not a DLG catalog. |
