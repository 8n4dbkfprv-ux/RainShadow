...
- Shading uses broad baked diffuse planes, low-resolution texture maps, restrained ambient occlusion, and limited muted color ramps. Generator masters are rasterised at the full 200-pixel texture body (V15: plate-density parity, 2.84 art-px per world unit against the office plate's 2.53 — a BG:EE sprite is never coarser than its background) with 1-bit alpha hardened at 50%, limited to per-material 128-entry ramps (derived from opaque pixels only) without dithering, plus a highlight-side value expansion toward BG paperdoll contrast; SpriteKit resolves them with linear filtering, which smooths the play-zoom magnification the same way the EE engine smooths its zoom. See `PaperdollBGEESpriteRedoPlanV15.md` for the density-parity rationale and `PaperdollBGEESpriteRedoPlanV14.md` for the earlier V7→V14 crunch. Avoid hand-placed pixels, coarse decorative pixel clusters, painterly brushwork, and modern pore/strand-level detail.
