...
| GemRB raw geometry | four triangles per 32×32 cell | cardinal edge interpolation plus separate diagonal smoothing |
| `FOGOWAR.BAM` | 32×32 N, W, NW + mirrors | fallback when the video driver cannot draw raw geometry |
...
RainShadow follows GemRB's raw-geometry path. It uploads one categorical state
texel per fog cell and evaluates `DrawFogCellVertices` / `DrawFogSmoothing` in a
fragment shader. The former 8×8-per-cell nearest-sampled mask was only a
performance approximation; outdoors it enlarged each baked fog texel into a
visible block and did not reproduce GemRB's separate blended passes.
