...
| Actors | adapters over `Movable` locomotion | `Gameplay/Actors/CutsceneActorAdapters.swift` |
...
- Do not bypass `NavigationMap` / `Movable` for cutscene actor floor movement.  
...
- **Floor movement goes through `NavigationMap` / `Movable`.** `.followPath` carries an authored
