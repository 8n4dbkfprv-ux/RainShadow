...
struct SearchMapTerrainTests {
    ...
    /// The bake reproduces the rectangles rather than reinterpreting them. If
    /// these diverge, either `bake_area_searchmap.py` or
    /// `SearchMap.rasterizeStaticObstacles` has changed and the other has not.
    @Test(arguments: AreaReachabilityTests.navigableIDs)
    func thePaintedMapAgreesWithTheRectanglesItWasBakedFrom(_ id: AreaID) throws {
        ...
        let painted = area.makeNavigationMap()
        var fromRects = area
        fromRects.searchMapName = nil
        let rectangles = fromRects.makeNavigationMap()

        #expect(painted.searchMap.columns == rectangles.searchMap.columns)
        #expect(painted.searchMap.rows == rectangles.searchMap.rows)

        var mismatches = 0
        for row in 0..<rectangles.searchMap.rows {
            for column in 0..<rectangles.searchMap.columns {
                let cell = SearchMapCell(column: column, row: row)
                let a = painted.searchMap.flags(at: cell).contains(.passable)
                let b = rectangles.searchMap.flags(at: cell).contains(.passable)
                if a != b { mismatches += 1 }
            }
        }
        #expect(mismatches == 0, "\(id) painted and rectangle rasters differ in \(mismatches) cells")
    }
    ...
    @Test func aSolidCellReportsNoSurfaceRatherThanASilentOne() throws {
        ...
        #expect(
            histogram.keys.allSatisfy { $0 == .obstacle || $0 == .stone },
            "the district raster carries terrain it was not baked with"
        )
    }
