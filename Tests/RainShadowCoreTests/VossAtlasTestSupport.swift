...
enum VossAtlasTestAssets {
    ...
    static func indexedManifestURL(character: String = "Voss") -> URL {
        if character == "Voss", ProcessInfo.processInfo.environment["RAINSHADOW_VOSS_ATLAS_ROOT"] != nil {
            return atlasRoot.appendingPathComponent("IEAvatar/Voss/\(IEIndexedSprite.manifestFileName)")
        }
        return repoRoot.appendingPathComponent("RainShadow Shared/Resources/Art/IE/Avatars/\(character)/\(IEIndexedSprite.manifestFileName)")
    }

    static func indexedSprite(character: String = "Voss") throws -> IEIndexedSprite {
        try IEIndexedSprite(contentsOf: indexedManifestURL(character: character), tables: IEGradientTables.load())
    }

    static var assetAuthority: String? {
        guard let data = try? Data(contentsOf: indexedManifestURL()),
              let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any] else { return nil }
        return json["asset_authority"] as? String
    }
    ...
    static var v20ManifestURL: URL? {
        ...
        let replacementCandidates = usesProjectionRegistration ? [
            atlasRoot.appendingPathComponent("voss_replacement_v14_manifest.json"),
            repoRoot.appendingPathComponent("ArtSource/Generated/Characters/Detective/ImportedVossReplacementRuntimeV14/voss_replacement_v14_manifest.json"),
            atlasRoot.appendingPathComponent("voss_replacement_v13_manifest.json"),
            repoRoot.appendingPathComponent("ArtSource/Generated/Characters/Detective/ImportedVossReplacementRuntimeV13/voss_replacement_v13_manifest.json"),
...
struct VossV20ValidationThresholds {
    ...
    /// Re-derived with the Super xBR enlargement; the manifest carries the
    /// shipped value and this is only the fallback. See
    /// `install_voss_v20.exact_gates` for the derivation.
    var seatedIdleNeutralIoUMinimum = 0.85
