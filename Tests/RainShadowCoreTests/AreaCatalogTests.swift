...
struct AreaCatalogTests {
    ...
    /// City block diamonds are the building mass, so some cell inside each
    /// cover outline is opaque. Office furniture cover is the WED split: the
    ...
    @Test func everyActorCoverStillHasSomethingOpaqueInIt() throws {
        for id in CityDistrictID.allCases {
            ...
            for cover in area.wallPolygons where cover.coversActors {
                ...
                #expect(opaque, "'\(id.slug)' '\(cover.id)' covers actors but blocks no sight")
            }
        }
    }
    ...
    @Test func everyShippedAreaResolvesToSomethingTheRouterCanBuild() {
        for id in HarborpointAreas.shippedIDs {
            let resolvable = id == HarborpointAreas.openingExterior
                ...
                || CityInteriorAreaAdapter.interior(for: id) != nil
    ...
    @Test func absentSectionsDecodeAsEmptyRatherThanFailing() throws {
        ...
        #expect(area.animations.isEmpty)
        #expect(area.variables.isEmpty)
        #expect(area.songs.isEmpty)
        #expect(area.lightMapName == nil)
        #expect(area.heightMapName == nil)
        #expect(area.resolvedLightMapName == "probe.lm")
        #expect(area.resolvedHeightMapName == "probe.ht")
