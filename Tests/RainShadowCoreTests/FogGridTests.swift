...
/// GemRB `FogRenderer` raw geometry: cardinal fan, diagonal smoothing, and the
/// two source-over alpha passes. These test the Swift reference used to grade
/// the runtime GLSL copy.
struct FogEdgeMaskTests {
    @Test func uniformCellsKeepTheThreeEngineLevels() {
        let grid = makeGrid()
        let all = allCells(in: grid)

        #expect(sample(grid, explored: [], visible: all, x: 0.5, y: 0.5) == 0)
        #expect(
            abs(sample(grid, explored: all, visible: [], x: 0.5, y: 0.5)
                - Double(FogGrid.rememberedLevel) / 255) < 0.000_001
        )
        #expect(sample(grid, explored: [], visible: [], x: 0.5, y: 0.5) == 1)
    }

    @Test func aCardinalShroudUsesTheFourTriangleFan() {
        let grid = makeGrid()
        var visible = allCells(in: grid)
        let north = FogCell(column: 1, row: 2)
        visible.remove(north)

        let peak = Double(FogGrid.rememberedLevel) / 255
        #expect(abs(sample(grid, explored: [north], visible: visible, x: 0.5, y: 1) - peak) < 0.000_001)
        #expect(abs(sample(grid, explored: [north], visible: visible, x: 0.5, y: 0.5) - peak) < 0.000_001)
        #expect(abs(sample(grid, explored: [north], visible: visible, x: 0.5, y: 0.25) - peak / 2) < 0.000_001)
        #expect(sample(grid, explored: [north], visible: visible, x: 0.5, y: 0) == 0)
    }

    @Test func aDiagonalOnlyShroudUsesTheSeparateCornerPass() {
        let grid = makeGrid()
        var visible = allCells(in: grid)
        let northEast = FogCell(column: 2, row: 2)
        visible.remove(northEast)

        let peak = Double(FogGrid.rememberedLevel) / 255
        #expect(abs(sample(grid, explored: [northEast], visible: visible, x: 1, y: 1) - peak) < 0.000_001)
        #expect(abs(sample(grid, explored: [northEast], visible: visible, x: 0.75, y: 0.75) - peak / 2) < 0.000_001)
        #expect(sample(grid, explored: [northEast], visible: visible, x: 0.5, y: 0.5) == 0)
    }

    @Test func aCardinalPassSuppressesItsIncidentDiagonalSmoothing() {
        let grid = makeGrid()
        var visible = allCells(in: grid)
        let north = FogCell(column: 1, row: 2)
        let northEast = FogCell(column: 2, row: 2)
        ...
        let alpha = sample(
            grid,
            explored: [north, northEast],
            visible: visible,
            x: 1,
            y: 1
        )
        #expect(abs(alpha - peak) < 0.000_001)
        #expect(alpha < 0.6, "an unsuppressed second draw would over-darken this corner")
    }

    @Test func shroudAndOpaquePassesCompositeSourceOverRatherThanMax() {
        let grid = makeGrid()
        var visible = allCells(in: grid)
        visible.remove(FogCell(column: 1, row: 2)) // unexplored north

        let halfOfShroud = (Double(FogGrid.rememberedLevel) / 255) / 2
        let expected = halfOfShroud + 0.5 * (1 - halfOfShroud)
        let alpha = sample(grid, explored: [], visible: visible, x: 0.5, y: 0.25)
        #expect(abs(alpha - expected) < 0.000_001)
        #expect(alpha > 0.6, "max would leave this at opaque-pass alpha 0.5")
    }

    @Test func outOfBoundsNeighboursAreOpaqueLikeBitmapGetAtFalse() {
        let grid = makeGrid()
        let visible = allCells(in: grid)
        let corner = FogCell(column: 0, row: 0)
        let levels = grid.mask(explored: [], visible: visible)
        let alpha = FogEdgeMask.alpha(
            cellLevels: levels,
    ...
    private func makeGrid() -> FogGrid {
        // 6×16 = 96 wide and 8×12 = 96 high: exactly 3×3 fog cells.
        FogGrid(origin: .zero, searchColumns: 6, searchRows: 8)
    }

    private func allCells(in grid: FogGrid) -> Set<FogCell> {
        Set((0..<grid.rows).flatMap { row in
            (0..<grid.columns).map { column in FogCell(column: column, row: row) }
        })
    }
    ...
    private func sample(
        ...
        FogEdgeMask.alpha(
            cellLevels: grid.mask(explored: explored, visible: visible),
            columns: grid.columns,
            rows: grid.rows,
            column: 1,
            row: 1,
