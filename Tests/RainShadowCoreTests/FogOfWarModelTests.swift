...
struct FogOfWarModelTests {
    ...
    /// The texture is categorical state only. Nothing paints a pool, feathers a
    /// ring, blurs a clip, or uploads an output-resolution fog picture.
    @Test func theMaskUploadsOneStateTexelPerCellRatherThanPaintingFog() throws {
        ...
        #expect(source.contains("let width = grid.columns"))
        #expect(source.contains("let height = grid.rows"))
        #expect(source.contains("rgbaScratch[destination] = stateLevels[source]"))
        #expect(source.contains("rgbaScratch[destination + 3] = 255"))
        for painted in [
            ...
            "texturePixelsPerCell", "FogEdgeMask.composite"
        ] {
            #expect(!source.contains(painted), "\(painted) is painting the fog again")
        }
    }

    /// The state lookup is nearest, but the edge is GemRB's raw geometry at
    /// fragment resolution — not nearest or linear reconstruction of a tiny
    /// raster mask.
    @Test func theEdgeIsGeometryRatherThanAFilterRamp() throws {
        let source = try read(Self.renderer)
        ...
        #expect(source.contains("SKShader(source: FogGeometryShader.source)"))
        #expect(source.contains("float fogCardinalGeometry("))
        #expect(source.contains("float fogCornerGeometry("))
        #expect(source.contains("float fogSourceOver("))
        #expect(source.contains("texture2D(u_texture, uv).r"))
        #expect(grid.contains("cellPixelSize = 32"))
        #expect(!grid.contains("texturePixelsPerCell"))
        #expect(edges.contains("FogRenderer::DrawFogCellVertices"))
        #expect(edges.contains("FogRenderer::DrawFogSmoothing"))
        #expect(edges.contains("sourceOver"))
        #expect(node.contains("shader = renderer.makeShader()"))
        #expect(!source.contains("filteringMode = .linear"))
        #expect(!source.contains("applyingGaussianBlur"))
    }
    ...
    @Test func theOfficeFloodsRoomsAndTheCityDoesNot() throws {
        ...
        // Door outlines are not a stroke-only overlay bolted onto the scene.
        // They are the IE two-pass draw in `HighlightOutlineLayer`, which the
        // scene only has to lift above a live door leaf — GemRB's doors are
        // tilemap background, so nothing of theirs ever covers an outline.
        #expect(gameArea.contains("highlightOutlineLayer.setDrawOrder"))
        #expect(!gameArea.contains("toggleDoorOutlineHighlight"))
        let outlineLayer = try read("RainShadow Shared/Core/Scene/HighlightOutlineLayer.swift")
        #expect(outlineLayer.contains("halfTransparent"))
        #expect(outlineLayer.contains("node.fillColor = SKColor("))
        #expect(outlineLayer.contains("node.strokeColor = SKColor("))
        #expect(outlineLayer.contains("isAntialiased = false"))
