...
struct CityWorldExtentTests {
    ...
    @Test func cityWorldExtentIsAnInfinityEngineOutdoorArea() {
        ...
        #expect(CityDistrictDefinition.worldArtSize == CGSize(width: 5_120, height: 3_840))
        #expect(CityDistrictDefinition.worldArtSize.width / 64 == 80)
        #expect(CityDistrictDefinition.worldArtSize.height / 64 == 60)
    }
    ...
    @Test func paintedCityDoorsSitInTheHumanBand() throws {
        ...
        for id in CityDistrictID.allCases {
            ...
            for door in area.doors {
                let height = try #require(
                    ...
                    "\(id) \(door.id) is missing a painted aperture"
                )
                let multiple = height / adult
                #expect(
                    CityDistrictLayout.Band.doorLeaf.contains(multiple),
                    "\(id) \(door.id) is \(multiple)x adult (band \(CityDistrictLayout.Band.doorLeaf))"
                )
                let box = try #require(door.paintedAperture)
                #expect(abs(box.h - height) < 0.01)
            }
        }
    }
    ...
    @Test func cityDoesNotUseRouteToHideSealedGeometry() {
        ...
        for id in CityDistrictID.allCases {
            ...
            let map = district.makeGrid()
            for portal in district.portals {
                // `reachesExactly` is the instrument here, not a non-empty
                // path: `FindPath` relocates a blocked goal itself, so a route
                // existing says only that *somewhere near* was reached. This
                // suite exists precisely to stop that hiding sealed geometry.
                #expect(
                    map.reachesExactly(from: district.actorStart, to: portal.approachPoint),
                    "\(id) \(portal.id) is not exactly reachable"
                )
