...
struct VossAtlasV20ValidationTests {
    ...
    @Test func everyIdleDirectionHoldsStillAndKeepsOnePalette() throws {
        ...
        for direction in authoredDirections + ["se"] {
            let names = (0..<4).map {
                String(format: "voss_standing_idle_%@_%02d.png", direction, $0)
            }
            let frames = try names.map { name in
            ...
            let semantic = try VossProjectionContract.heads(atlas: "VossIdle.atlas", names: names)
            let headWidths = semantic?.map(\.width) ?? metrics.map { Double($0.headWidth) }
            ...
            // One palette per clip: a loop carrying more entries than a palette
            // has is one whose frames were quantised apart, which is what made
            // the wardrobe shift between phases.
            //
            // Counted on the *index plane*, not the compatibility atlas. Runtime
            // presentation may sample the native plane linearly, while legacy
            // fallback atlases may be prefiltered; neither is the palette craft
            // being gated here. The plane still uses a subset of 84 body entries.
            var clipIndices = Set<UInt8>()
            for name in names {
                guard let indexed = sprite.frame(atlas: "VossIdle.atlas", name: name) else {
                    continue
                }
                clipIndices.formUnion(indexed.indices.filter { $0 != UInt8(IEPalette.colorKeyIndex) })
            }
            #expect(
                clipIndices.count <= thresholds.maximumOpaqueColors,
                "idle \(direction) carries \(clipIndices.count) distinct palette entries across 4 phases, expected <=\(thresholds.maximumOpaqueColors)"
            )
        }
    }
    ...
    private func validateRasterAndGeometry(
        ...
        // The per-frame palette budget moved onto the index plane with the clip
        // budget above; see the note there. What the atlas still owes is its
        // alpha contract, asserted immediately above.
    ...
    private func validateMotion(
        ...
        let metrics = try frames.map { try $0.metrics() }
        let names = (0..<8).map { String(format: "voss_walk_%@_%02d.png", direction, $0) }
        let semantic = try VossProjectionContract.heads(atlas: "VossWalk.atlas", names: names)
        let headCenters = semantic?.map(\.x) ?? metrics.map(\.headCenterX)
