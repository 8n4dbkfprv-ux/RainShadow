...
struct DialogueApproachTests {
    ...
    /// The whole reason this uses `reachesExactly`: `FindPath` relocates a blocked goal
    /// itself, so a non-empty path only says the actor reached *somewhere near* what was
    /// asked for — which would park the detective on the far side of a wall from the
    /// person he is supposedly talking to.
    @Test func refusesAnActorItCannotActuallyReach() {
        ...
        // Every standing spot around the actor is on the far side of the wall.
        for candidate in DialogueApproach.candidates(
            around: actor, facing: walker, standoff: 15
        ) {
            #expect(!map.reachesExactly(from: walker, to: candidate))
        }
    }
