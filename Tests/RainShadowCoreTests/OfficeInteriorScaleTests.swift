...
struct OfficeInteriorScaleTests {
    ...
    /// Floor diamond is a true BG:EE ±0.75 parallelogram inside the V20 paint.
    ///
    /// V20 re-registers the approved painting onto the BG:EE lock with one
    /// vertical-preserving affine, then derives the navigation diamond: the
    /// maximum-area exact ±0.75 parallelogram anchored at the painted rear
    /// floor corner and inscribed in the warped painted-floor quad. The
    /// runtime's `verticalProjectionScale`, the 16×12 search map and the 16:12
    /// rings are all hard-locked at 0.75, so any other slope here is a bug.
    ...
    @Test func floorDiamondIsFittedToThePaintedFloor() {
        ...
        // Exact BG:EE ground axes. Authored y-up flips the y-down slopes.
        #expect(abs(arch.axisNW.dy / arch.axisNW.dx + 0.75) < 0.0005)
        #expect(abs(arch.axisNE.dy / arch.axisNE.dx - 0.75) < 0.0005)
        ...
        #expect(abs(rearYDown - paintTopYDown) <= 2, "rear corner left the painted rear floor corner")
        #expect(paintBottomYDown < artHeight, "near tip fell off the plate")
        #expect(abs(paintBottomYDown - 2_150) <= 2, "near tip left the V20 painted floor")
    ...
    @Test func entranceRegistrationMatchesV20DoorPolygonFit() {
        ...
        #expect(abs(entrance.x - 2_439.9429353470478) < 0.001)
        // V20 keeps the V11 leaf pixels, scale, and hinge, then registers
        // the assembly onto the painted door polygon of the warped plate.
        #expect(abs(entrance.y - 757.1778341796478) < 0.001)
    ...
    @Test func entranceDoorRegistersToShippingAperture() {
        ...
        #expect(abs(leaf.x - 2_640.9441756999454) < 0.001)
        #expect(abs(leaf.y - 848.1632185194262) < 0.001)
        #expect(
        ...
        for blocking in [true, false] {
            ...
            for (hotspotID, destination) in OfficeNavigationLayout.approachPoints {
                ...
                if hotspotID == "office.door" || hotspotID == "office.window" {
                    ...
                    // The engine terminates on the search node inside the goal
                    // cell, so an approach lands in the right cell rather than on
                    // the authored point exactly. Same cell is the contract.
                    #expect(
                        route.map {
                            map.searchMap.cell(for: $0.resolvedDestination)
                                == map.searchMap.cell(for: destination)
                        } == true,
                        "Approach \(hotspotID) resolved out of its authored cell"
                    )
                }
            }
        }
    ...
    @Test func emptyDeskChairMatchesSeatedDeskNudge() {
        ...
        #expect(nudge == CGPoint(x: -20, y: 0))
        #expect(OfficeInteriorScale.ActorDisplay.seatedUpperDeskReach == .zero)
    ...
    @Test func pathAcrossDoorwayIsBlockedOrRoutedAround() {
        ...
        // `AdjustPositionDirected` runs inside `FindPath`, so an order onto a
        // solid is answered with a route to the near side of it rather than a
        // refusal. What must never happen is *ending on* the leaf.
        #expect(
            !grid.reachesExactly(from: OfficeNavigationLayout.actorStart, to: onDoor),
            "Must not pathfind onto the door leaf"
        )
    }
    ...
    @Test func pathOntoDeskIsNil() {
        ...
        for point in OfficeNavigationLayout.deskSamplePoints {
            #expect(
                !grid.reachesExactly(from: OfficeNavigationLayout.actorStart, to: point),
                "Must not pathfind onto desk sample \(point)"
            )
        }
    }
    ...
    @Test func pathOntoMajorPropsIsNil() {
        ...
        for point in OfficeNavigationLayout.majorPropSamplePoints {
            #expect(
                !grid.reachesExactly(from: OfficeNavigationLayout.actorStart, to: point),
                "Must not pathfind onto prop sample \(point)"
            )
        }
    }
    ...
    @Test func hotspotApproachesStayOutsideObstaclesAndAvoidThemOnPath() {
        ...
        for (hotspotID, destination) in OfficeNavigationLayout.approachPoints {
            ...
            // The final node is the search node inside the goal cell, so it can
            // sit a few units off the authored approach; only the legs between
            // are asserted clear of solids.
            #expect(
                route?.waypoints.dropLast().allSatisfy {
                    !OfficeNavigationLayout.isBlocked($0)
                } == true,
    ...
    @Test func pathsThatWouldCrossTheDeskNeverEnterOfficeObstacles() {
        ...
        for (rawStart, rawEnd) in rawPairs {
            ...
            if path.isPresent {
                ...
                #expect(points.allSatisfy { !OfficeNavigationLayout.isBlocked($0) },
                        "Path \(start)->\(end) placed a waypoint inside an obstacle")
                #expect(points.allSatisfy { !desk.contains($0) },
                        "Path \(start)->\(end) placed a waypoint inside the desk")
            }
            // An empty path is acceptable when the corridor is fully sealed; a
            // route that does exist must stay clear.
        }
        #expect(exercised >= 1, "Expected at least one successful around-desk path to inspect")
    }

    @Test func officeCameraShowsTheNativeBodyAtHundredPercent() {
        let bodyPoints = DefaultPlayZoom.displayedBodyHeight(
        ...
        #expect(bodyPoints == 64)

        // A large view may outgrow the plate. Never fit or inflate the room to
        // compensate; AreaViewport owns the same over-large clamp as GemRB.
        let shellFill = OfficeInteriorScale.scaledArtSize.height
            / OfficeInteriorScale.cameraVisibleHeight(forSceneHeight: 1_152)
        #expect(shellFill < 1)
